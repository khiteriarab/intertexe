...
import { ArrowLeft, Leaf, Droplets, Shield, Sparkles } from "lucide-react";
...
export default function MaterialDetail() {
  ...
  return (
    <div className="py-6 md:py-12 flex flex-col gap-10 md:gap-16 max-w-4xl mx-auto" data-testid={`page-material-${material.slug}`}>
      ...
      <section className="flex flex-col gap-6 md:gap-8">
        <div className="flex flex-col gap-2">
          <h2 className="text-2xl md:text-3xl font-serif">Designers Known for {material.name}</h2>
          <p className="text-muted-foreground text-sm">Brands with strong commitments to natural fiber quality.</p>
        </div>
            ...
            {relatedDesigners.map((designer: any) => (
              <Link key={designer.id} href={`/designers/${designer.slug}`} className="group flex flex-col gap-3" data-testid={`card-designer-${designer.slug}`}>
                <div className="aspect-[3/4] bg-secondary relative flex items-center justify-center">
                  <span className="font-serif text-3xl md:text-4xl text-muted-foreground/30">{designer.name.charAt(0)}</span>
                </div>
                <div>
