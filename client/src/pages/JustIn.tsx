    ...
    <div className="py-8 md:py-12 flex flex-col gap-16">
      <header className="flex flex-col gap-4">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs uppercase tracking-widest text-muted-foreground">Latest Additions</span>
        </div>
        <h1 className="text-4xl md:text-6xl font-serif" data-testid="text-just-in-title">Just In</h1>
      ...
      {isLoading ? (
        ...
        <>
          {featured && (
            <Link href={`/designers/${featured.slug}`} className="group" data-testid={`card-featured-${featured.slug}`}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 border border-border/40 group-hover:border-foreground/20 transition-colors">
                <div className="aspect-[4/3] md:aspect-auto bg-secondary relative overflow-hidden flex items-center justify-center">
                  <span className="font-serif text-[120px] text-muted-foreground/10">{featured.name.charAt(0)}</span>
                </div>
          ...
          {recent.length > 0 && (
            <section className="flex flex-col gap-8">
              <h2 className="text-2xl font-serif border-b border-border/40 pb-4">Recently Added</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                {recent.map((designer: any) => (
                  <Link key={designer.id} href={`/designers/${designer.slug}`} className="group flex flex-col gap-4" data-testid={`card-recent-${designer.slug}`}>
          ...
          {remaining.length > 0 && (
            <section className="flex flex-col gap-8">
              <h2 className="text-2xl font-serif border-b border-border/40 pb-4">More Designers</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {remaining.map((designer: any) => (
                  <Link key={designer.id} href={`/designers/${designer.slug}`} className="group flex items-center justify-between p-5 border border-border/40 hover:border-foreground/30 transition-colors" data-testid={`card-more-${designer.slug}`}>
                    <div className="flex flex-col gap-1">
                      <h3 className="text-lg font-serif group-hover:text-muted-foreground transition-colors">{designer.name}</h3>
