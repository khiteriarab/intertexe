-- Replace The White Edit with The Leather Edit (September 2026).
-- After apply: rebuild leather-edit memberships + refresh collections:leather-edit rail.
...
SELECT
  ...
  enabled,
  sort_order,
  coalesce(notes, '') || ' Renamed from white-edit Sep 2026'
FROM public.homepage_merch_rails
WHERE rail_key = 'collections:white-edit'
...
SET rail_key = 'collections:leather-edit'
WHERE rail_key = 'collections:white-edit';
...
SET collection_slugs = (
  ...
  FROM (
    SELECT CASE
      WHEN s IN ('white-edit', 'the-white-edit', 'the-leather-edit') THEN 'leather-edit'
      ELSE s
    END AS mapped
    FROM unnest(collection_slugs) AS s
  ) x
)
WHERE collection_slugs && ARRAY['white-edit', 'the-white-edit', 'the-leather-edit']::text[];

DO $$
BEGIN
  IF to_regclass('public.collection_product_memberships') IS NOT NULL THEN
    UPDATE public.collection_product_memberships
    SET collection_slug = 'leather-edit'
    WHERE collection_slug IN ('white-edit', 'the-white-edit');
  END IF;
END $$;
...
AS $$
  SELECT CASE public.editorial_resolve_collection_slug(p_slug)
    ...
    WHEN 'white-edit' THEN 'leather-edit'
    WHEN 'the-white-edit' THEN 'leather-edit'
    WHEN 'the-leather-edit' THEN 'leather-edit'
...
BEGIN
  ...
  IF slugs && ARRAY['white-edit','the-white-edit','leather-edit','the-leather-edit']::text[] THEN keys := keys || 'collections:leather-edit'; END IF;
