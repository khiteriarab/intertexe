...
function AccountDashboard({ user, onLogout }: { user: UserData; onLogout: () => void }) {
  ...
  useEffect(() => {
    const token = getToken();
    if (!token) { setFavsLoading(false); return; }
    fetch("/api/favorites", { headers: { Authorization: `Bearer ${token}` } })
      ...
      .then(async (data) => {
        ...
        const designerIds = data.map((f: any) => f.designerId).filter(Boolean);
        if (designerIds.length > 0) {
          try {
            const res = await fetch(`/api/designers?ids=${designerIds.join(",")}`);
            if (res.ok) {
              const designers = await res.json();
              const designerMap = new Map<string, any>();
              (Array.isArray(designers) ? designers : []).forEach((d: any) => {
                designerMap.set(String(d.id), d);
              });
              const enriched = data.map((f: any) => ({
                ...f,
                designer: designerMap.get(String(f.designerId)) || null,
              }));
  ...
  const removeFav = async (designerId: string) => {
    const token = getToken();
    if (!token) return;
    try {
      await fetch(`/api/favorites/${encodeURIComponent(designerId)}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
        ...
        <div className="flex md:flex-col gap-1.5 md:gap-1 -mx-4 px-4 md:mx-0 md:px-0">
          {menuItems.map(item => (
            <button key={item.id} onClick={() => handleTabChange(item.id)}
              ...
              {item.id === "wishlist" && (favorites.length + productFavCount) > 0 && (
                <span className="ml-auto text-[9px] opacity-60 hidden md:inline">{favorites.length + productFavCount}</span>
              )}
              {item.id === "quiz" && quizResults.length > 0 && (
                <span className="ml-auto text-[9px] opacity-60 hidden md:inline">{quizResults.length}</span>
              )}
            </button>
          ))}
        </div>
      ...
      <div ref={contentRef} className="flex-1 flex flex-col gap-8 md:gap-12 scroll-mt-20">
            ...
            <div className="flex flex-col gap-0">
              ...
              >
                ...
                <span className="flex items-center gap-2">
                  <span className="text-sm">{favorites.length + productFavCount}</span>
        ...
        {activeTab === "wishlist" && (
          <section className="flex flex-col gap-5 md:gap-6 animate-in fade-in duration-300">
            ...
            {productFavCount > 0 && (
              <div className="flex flex-col gap-3">
                <div className="flex items-center justify-between">
                  ...
                  {(() => {
                    const dropCount = savedProducts.filter((p: any) => getPriceDrop(String(p.id), p.price)).length;
                    return dropCount > 0 ? (
            ...
            {favorites.length > 0 && (
              <div className="flex flex-col gap-3">
                <h3 className="text-sm uppercase tracking-[0.15em] text-muted-foreground">Saved Designers ({favorites.length})</h3>
                    ...
                    return (
                      <Link key={fav.id} href={`/designers/${fav.designer?.slug || fav.designerId}`} className="group flex flex-col border border-border hover:border-foreground transition-colors active:scale-[0.98] overflow-hidden" data-testid={`card-favorite-${fav.designer?.slug || fav.designerId}`}>
                        <div className="aspect-[4/3] w-full bg-secondary flex items-center justify-center">
                          <span className="text-2xl font-serif text-muted-foreground/40">{brandName.charAt(0)}</span>
                        </div>
                        <div className="flex flex-col gap-2 p-4">
                          <div className="flex justify-between items-start">
                            <h3 className="text-base md:text-lg font-serif">{brandName}</h3>
                            <button className="text-foreground p-1.5 -mr-1 active:scale-90 transition-transform" onClick={(e) => { e.preventDefault(); removeFav(fav.designerId); }}>
            ...
            {favorites.length === 0 && productFavCount === 0 && (
