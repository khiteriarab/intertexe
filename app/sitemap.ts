import type { MetadataRoute } from "next";
import { COLLECTION_SLUGS } from "../lib/collection-pages";

const BASE = "https://www.intertexe.com";

export default function sitemap(): MetadataRoute.Sitemap {
  const now = new Date();
  const materials = ["silk", "cashmere", "linen", "wool", "cotton"] as const;

  const staticPages: MetadataRoute.Sitemap = [
    { url: BASE, lastModified: now, changeFrequency: "daily", priority: 1 },
    { url: `${BASE}/shop`, lastModified: now, changeFrequency: "daily", priority: 0.9 },
    { url: `${BASE}/sale`, lastModified: now, changeFrequency: "daily", priority: 0.8 },
    { url: `${BASE}/designers`, lastModified: now, changeFrequency: "weekly", priority: 0.7 },
    { url: `${BASE}/materials`, lastModified: now, changeFrequency: "weekly", priority: 0.7 },
    { url: `${BASE}/scanner`, lastModified: now, changeFrequency: "monthly", priority: 0.6 },
    { url: `${BASE}/about`, lastModified: now, changeFrequency: "monthly", priority: 0.45 },
    { url: `${BASE}/press`, lastModified: now, changeFrequency: "monthly", priority: 0.4 },
    { url: `${BASE}/press-kit`, lastModified: now, changeFrequency: "monthly", priority: 0.4 },
    // B2B / SaaS marketing
    { url: `${BASE}/brands`, lastModified: now, changeFrequency: "weekly", priority: 0.85 },
    { url: `${BASE}/digital-product-passport`, lastModified: now, changeFrequency: "weekly", priority: 0.9 },
    { url: `${BASE}/brands/solutions`, lastModified: now, changeFrequency: "monthly", priority: 0.55 },
    { url: `${BASE}/brands/pricing`, lastModified: now, changeFrequency: "monthly", priority: 0.55 },
    { url: `${BASE}/brands/demo`, lastModified: now, changeFrequency: "weekly", priority: 0.7 },
    { url: `${BASE}/brands/request`, lastModified: now, changeFrequency: "monthly", priority: 0.4 },
    { url: `${BASE}/brands/success-stories`, lastModified: now, changeFrequency: "monthly", priority: 0.45 },
    { url: `${BASE}/brands/product-intelligence`, lastModified: now, changeFrequency: "monthly", priority: 0.5 },
    { url: `${BASE}/brands/traceability`, lastModified: now, changeFrequency: "monthly", priority: 0.5 },
    { url: `${BASE}/brands/environmental-intelligence`, lastModified: now, changeFrequency: "monthly", priority: 0.5 },
    { url: `${BASE}/brands/connected-product-lifecycle`, lastModified: now, changeFrequency: "monthly", priority: 0.5 },
    { url: `${BASE}/brands/supplier-data`, lastModified: now, changeFrequency: "monthly", priority: 0.5 },
  ];

  const collectionPages: MetadataRoute.Sitemap = COLLECTION_SLUGS.map((slug) => ({
    url: `${BASE}/collections/${slug}`,
    lastModified: now,
    changeFrequency: "weekly" as const,
    priority: 0.8,
  }));

  const materialPages: MetadataRoute.Sitemap = materials.map((fiber) => ({
    url: `${BASE}/materials/${fiber}`,
    lastModified: now,
    changeFrequency: "weekly" as const,
    priority: 0.7,
  }));

  return [...staticPages, ...collectionPages, ...materialPages];
}
