...
export function AppStoreSyncButton({
  connected,
  downloadsReady,
  syncedAt,
}: {
  ...
  async function sync() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/dashboard/integrations/app_store_connect", {
        method: "POST",
      ...
      if (!res.ok || json.ok === false) {
        throw new Error(json.error || json.message || "App Store sync failed");
      }
      router.refresh();
    } catch (e) {
  ...
  return (
    <span className="inline-flex items-center gap-2 shrink-0">
      ...
      >
        {busy ? "Updating…" : "Refresh from Apple"}
      </button>
    </span>
  );
}
