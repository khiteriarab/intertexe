...
export function EmailBanner() {
  ...
  return (
    <div
      className="fixed bottom-0 left-0 right-0 z-[60] bg-[#111] text-white shadow-2xl"
        ...
        {status === "success" ? (
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-6">
            <div className="flex-1 flex items-center gap-3 min-w-0">
              <p className="text-[12px] md:text-[13px] font-light leading-snug">
            ...
            <form onSubmit={handleSubmit} className="flex gap-2 flex-shrink-0">
              <input
                ...
                required
                className="w-full md:w-[220px] px-3 py-2.5 bg-white/10 border border-white/20 text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-white/50 transition-colors"
                data-testid="input-banner-email"
              />
              <button
                type="submit"
                disabled={status === "loading"}
                className="px-5 py-2.5 bg-white text-[#111] text-[10px] uppercase tracking-[0.15em] font-medium hover:bg-white/90 transition-colors disabled:opacity-50 flex-shrink-0"
