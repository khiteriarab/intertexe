      ...
      {productsWithImages.length > 0 && (
        <section className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4 mb-12 md:mb-16">
          {productsWithImages.slice(0, 24).map((product, i) => (
            <Link key={product.id} href={`/product/${product.id}`} className="group bg-background border border-border/20 hover:border-border/60 transition-all flex flex-col" data-testid={`card-category-product-${i}`}>
              <div className="aspect-[3/4] bg-secondary relative overflow-hidden">
                ...
                {product.naturalFiberPercent >= 90 && (
                  <div className="absolute top-2 left-2">
                    <span className="bg-emerald-900/90 text-emerald-100 px-2 py-0.5 text-[8px] uppercase tracking-[0.1em] font-medium backdrop-blur-sm">{product.naturalFiberPercent >= 95 ? "100% Natural" : `${product.naturalFiberPercent}% Natural`}</span>
                  </div>
                )}
              </div>
