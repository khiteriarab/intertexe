...
export function DesignersAllClient({ designers }: { designers: DesignerItem[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [designers, search, tierFilter]);

  const { grouped, sortedKeys } = useMemo(() => {
    const g = filtered.reduce((acc: Record<string, DesignerItem[]>, designer) => {
  ...
  return (
    <>
      <header className="flex flex-col items-center text-center gap-4 md:gap-6 max-w-2xl mx-auto">
        ...
        <h1 className="text-3xl md:text-5xl font-serif" style={{ fontFamily: "Playfair Display, serif" }} data-testid="text-all-brands-title">All Brands</h1>
        <p className="text-muted-foreground text-sm md:text-base">
          Browse our full database of {designers.length.toLocaleString()}+ brands, A to Z.
        </p>
          ...
          {sortedKeys.length === 0 ? (
            ...
            sortedKeys.map((letter) => (
              <section key={letter} id={`letter-${letter}`} className="flex flex-col gap-4 md:gap-6 scroll-mt-24">
                <h2 className="text-3xl md:text-4xl font-serif border-b border-border/40 pb-3 md:pb-4 text-foreground/80" style={{ fontFamily: "Playfair Display, serif" }}>
