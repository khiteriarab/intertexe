...
export { consumerExclusionReason, passesConsumerIngestionGate, isWomensApparelCategory };
...
function normGarmentText(value) {
  return String(value || '').toLowerCase().trim();
}

/** Underwear / intimates — must win over swim when retailers tag "swimwear" on lingerie bikinis. */
export function isLingerieGarment(category = '', name = '') {
  const cat = normGarmentText(category);
  const nam = normGarmentText(name);
...
/** Sleep / lounge — exclude French "Robe … Dress" naming. */
export function isSleepwearGarment(category = '', name = '') {
  const cat = normGarmentText(category);
  const nam = normGarmentText(name);
...
/** Swim / beach — never steal lingerie bikinis. */
export function isSwimGarment(category = '', name = '') {
...
export function catalogRegionRank(region, preferred) {
  ...
  if (r === p) return 0;
  if (r === 'uk') return p === 'eu' ? 1 : 2;
  if (r === 'us') return p === 'eu' ? 2 : p === 'uk' ? 1 : 1;
  if (r === 'eu') return p === 'eu' ? 0 : 3;
  return 4;
}
