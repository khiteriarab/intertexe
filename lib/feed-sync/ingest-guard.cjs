 ...
 *
 * Stage-only dry runs (FEED_STAGE_DRY_RUN=1 + FEED_STAGE_ONLY) may write to
 * feed_staging_* even while kill switches remain armed. They never write products.
 *
...
function stageDryRunEnvEnabled() {
  return String(process.env.FEED_STAGE_DRY_RUN || "").trim() === "1";
}
...
async function assertIngestAllowed(supabase, opts = {}) {
  ...
  // Live path: both kill switches must be clear.
  if (wantLive) {
  ...
  // Stage path: allowed while switches are armed only for explicit dry runs.
  if (stageOnlyEnvEnabled()) {
    if (
      (publish.blocked === true || ingest.blocked === true) &&
      !stageDryRunEnvEnabled()
    ) {
      return {
        ok: false,
        mode: "blocked",
        reason:
          "kill_switches_armed_set_FEED_STAGE_DRY_RUN=1_for_staging_only_or_clear_switches",
      };
    }
    return {
      ok: true,
      mode: "stage",
      reason: stageDryRunEnvEnabled()
        ? "stage_dry_run_while_switches_armed"
        : "stage_only",
    };
  }
