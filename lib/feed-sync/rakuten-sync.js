...
const MID_CURRENCY_MAP = {
  ...
  // Peachy Den (UK) — MID 52708
  '52708': { currency: 'GBP', region: 'uk', retailerCountry: 'GB' },
};
...
function normalizeRakutenProduct(row, feedUrl) {
  ...
  const apparelOk = passesConsumerIngestionGate({ category, name, composition, imageUrl, price, url, gender });
  const footwearOk = passesFootwearIngestionGate({ category, name, composition, imageUrl, price, url, gender });
  ...
  const normalized = {
    ...
    is_sale:               isSale(resolvedPrice, originalPrice, row),
    ...
    feed_source:           mytheresa ? 'mytheresa' : (footwearOk && !apparelOk ? 'rakuten_footwear' : 'rakuten'),
...
function isSale(price, originalPrice, row) {
  ...
  if (category && /\bsale\b|\boutlet\b|\bmarkdown\b/.test(category)) return true;
