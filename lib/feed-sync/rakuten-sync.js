import { createClient } from '@supabase/supabase-js';
import { XMLParser } from 'fast-xml-parser';
import { Client as FtpClient } from 'basic-ftp';
import { execSync } from 'child_process';
import { createRequire } from 'module';
import { PassThrough, Readable } from 'stream';
import { createGunzip, gunzipSync } from 'zlib';
import { classifyGarment } from '../catalog-rules.js';
import { extractColorFromName } from './color-from-name.js';
import {
  MYTHERESA_MIDS,
  BLOOMINGDALES_MIDS,
  RAKUTEN_MID_CURRENCY_MAP,
  merchantByMid,
} from './rakuten-merchants.js';

const requireCjs = createRequire(import.meta.url);
const { assertIngestAllowed } = requireCjs('./ingest-guard.cjs');
const staging = requireCjs('./staging.cjs');

const NATURAL_FIBERS = ['cotton', 'linen', 'silk', 'wool', 'cashmere', 'alpaca', 'mohair', 'hemp', 'ramie', 'leather', 'suede'];
const DEFAULT_BATCH_SIZE = parseInt(process.env.FEED_SYNC_BATCH_SIZE || '100', 10);
const INACTIVE_CHUNK_SIZE = 500;
/** Flush FTP rows to DB in small waves so a single merchant file cannot OOM the runner. */
const FTP_ROW_FLUSH_SIZE = parseInt(process.env.FEED_SYNC_FTP_FLUSH_SIZE || '250', 10);
/**
 * Merchant files that previously exhausted GitHub Actions memory (~7GB RAM).
 * Skip them so the rest of the catalog can advance; process offline later.
 */
const DEFAULT_SKIP_FTP_FILES = ['50739_4668007_mp.xml.gz'];
const SKIP_FTP_FILES = new Set(
  [
    ...DEFAULT_SKIP_FTP_FILES,
    ...(process.env.RAKUTEN_SKIP_FTP_FILES || '')
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean),
  ].map((s) => s.toLowerCase())
);

const VERONICA_BEARD_CA_MIDS = new Set(['52963']);

// MID → forced currency/region (overrides feed-level XML values)
const MID_CURRENCY_MAP = RAKUTEN_MID_CURRENCY_MAP;

const MYTHERESA_BRAND_FIELD_KEYS = [
  'brand',
  'brand_name',
  'designer',
  'designer_name',
  'label',
  'advertiser_name',
  'merchant_name',
  'google_custom_label_0',
  'custom_label_0',
  'product_type',
];

/** Designer brand on Bloomingdale's marketplace rows — never the retailer name. */
const BLOOMINGDALES_BRAND_FIELD_KEYS = [
  'brand',
  'brand_name',
  'designer',
  'designer_name',
  'manufacturer',
  'manufacturer_name',
  'label',
];

function isVeronicaBeardCanadaRow(row) {
  const mid = first(row, ['mid', 'merchant_id', 'advertiser_id', 'network_mid']);
  return VERONICA_BEARD_CA_MIDS.has(String(mid || '').trim());
}

function isMytheresaRow(row, feedUrl = '') {
  const mid = first(row, ['mid', 'merchant_id', 'advertiser_id', 'network_mid']);
  if (mid && MYTHERESA_MIDS.has(String(mid).trim().toLowerCase())) return true;
  if (feedUrl.toLowerCase().includes('mytheresa')) return true;
  const advertiser = String(first(row, ['advertiser_name', 'merchant_name']) || '').toLowerCase();
  return advertiser.includes('mytheresa');
}

function isBloomingdalesRow(row, feedUrl = '') {
  const mid = first(row, ['mid', 'merchant_id', 'advertiser_id', 'network_mid']);
  if (mid && BLOOMINGDALES_MIDS.has(String(mid).trim())) return true;
  const feedLower = feedUrl.toLowerCase();
  if (feedLower.includes('13867') || feedLower.includes('37206') || feedLower.includes('bloomingdale')) return true;
  const url = String(first(row, ['link', 'url', 'product_url', 'click_url', 'buy_url']) || '').toLowerCase();
  if (url.includes('bloomingdales.com') || url.includes('bloomingdales.co.uk')) return true;
  const advertiser = String(first(row, ['advertiser_name', 'merchant_name', 'retailer']) || '').toLowerCase();
  return advertiser.includes('bloomingdale');
}

function isBloomingdalesRetailerMid(mid) {
  const id = String(mid || '').trim();
  return id === '13867' || id === '37206';
}

async function withRetry(fn, retries = 3, delayMs = 500) {
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      return await fn();
    } catch (err) {
      const isTransient = err.message?.includes('fetch failed') || err.message?.includes('ECONNRESET') || err.message?.includes('socket') || err.code === 'ETIMEDOUT'
        || err.code === '57014' || err.message?.includes('statement timeout') || err.message?.includes('canceling statement')
        || err.status === 429 || err.message?.includes('Too Many Requests');
      if (!isTransient || attempt === retries) throw err;
      await new Promise((r) => setTimeout(r, delayMs * Math.pow(2, attempt)));
    }
  }
}

async function syncRakutenFeeds(options = {}) {
  const config = readConfig(options);
  const supabase = createClient(config.supabaseUrl, config.supabaseServiceRoleKey, {
    auth: { persistSession: false }
  });
  const syncStartedAt = new Date().toISOString();

  // P0: never mutate live products unless ingest is explicitly armed and kill switches are clear.
  const ingestGate = await assertIngestAllowed(supabase, {
    forceLive: options.forceLive === true,
  });
  if (!ingestGate.ok) {
    console.error('[P0] ingest blocked:', ingestGate.reason);
    return {
      upserted: 0,
      rejected: 0,
      newProducts: 0,
      updatedProducts: 0,
      filesProcessed: 0,
      totalCatalogFiles: 0,
      inactiveMarked: 0,
      stockStatusBackfilled: 0,
      colorUpdatesApplied: 0,
      errors: [{ feedUrl: 'ingest_guard', message: ingestGate.reason }],
      ingestBlocked: true,
      ingestMode: ingestGate.mode,
    };
  }
  const stageMode = ingestGate.mode === 'stage';
  let stagingSession = null;
  const liveArmed = String(process.env.FEED_LIVE_INGEST_ENABLED || '').trim() === '1';
  if (stageMode) {
    stagingSession = await staging.openOrResumeStagingSession(supabase, {
      openedAt: syncStartedAt,
      fileOffset: config.fileOffset,
    });
    // Never point writes at live products in stage mode.
    config.productsTable = '__stage_only_do_not_use__';
    config.markInactive = false;
  } else if (String(config.productsTable || 'products') === 'products' && !liveArmed) {
    throw new Error('P0: refusing live products write without FEED_LIVE_INGEST_ENABLED=1');
  }

  let fetched = 0;
  let normalized = 0;
  let skippedOutOfScope = 0;
  let upserted = 0;
  let colorUpdatesApplied = 0;
  let compositionUpdatesApplied = 0;
  const errors = [];
  let filesProcessed = 0;
  let totalCatalogFiles = 0;
  const stats = {
    totalProcessed: 0,
    newProducts: 0,
    updatedProducts: 0,
    nowApproved: 0,
    rejected: 0,
    missingComposition: 0,
    skippedOutOfScope: 0,
    errors: [],
  };

  if (config.fileOffset === 0 && !config.skipFtp) {
    try {
      await supabase.from('system_status').upsert({
        key: 'rakuten_feed_sync_cycle',
        value_json: { cycleStartedAt: syncStartedAt, ingestMode: ingestGate.mode },
        updated_at: syncStartedAt,
      });
    } catch (cycleErr) {
      errors.push({ feedUrl: 'cycle_start', message: cycleErr.message });
    }
  }

  async function processRows(rows, sourceUrl) {
    const maxUpserts = Number(process.env.FEED_SYNC_MAX_UPSERTS || 0);
    if (maxUpserts > 0 && upserted >= maxUpserts) {
      return;
    }
    fetched += rows.length;
    const products = [];
    const colorCandidates = [];
    const compositionCandidates = [];
    const COLOR_FLUSH_SIZE = 500;
    const COMPOSITION_FLUSH_SIZE = 500;

    for (const row of rows) {
      if (maxUpserts > 0 && upserted + products.length >= maxUpserts) break;
      const gateFields = extractRowGateFields(row, sourceUrl);
      const exclusion = consumerExclusionReason(gateFields);
      if (exclusion === 'missing_composition') {
        stats.missingComposition += 1;
      }
      const product = normalizeRakutenProduct(row, sourceUrl);
      if (product) {
        products.push(product);
        stats.totalProcessed += 1;
      } else {
        skippedOutOfScope += 1;
        stats.skippedOutOfScope += 1;
        stats.rejected += 1;

        const compUpdate = extractCompositionUpdateCandidate(row, sourceUrl);
        if (compUpdate && !stageMode) {
          compositionCandidates.push(compUpdate);
          if (compositionCandidates.length >= COMPOSITION_FLUSH_SIZE) {
            compositionUpdatesApplied += await applyCompositionUpdates(
              supabase,
              config.productsTable,
              compositionCandidates.splice(0, compositionCandidates.length)
            );
          }
          continue;
        }

        if (exclusion !== 'missing_composition') {
          continue;
        }

        // mp.xml rows without composition — stub for later cmp merge.
        if (!/_cmp/i.test(String(sourceUrl || ''))) {
          const stub = normalizeRakutenProductStub(row, sourceUrl);
          if (stub) {
            products.push(stub);
            stats.totalProcessed += 1;
            stats.skippedOutOfScope -= 1;
            stats.rejected -= 1;
            continue;
          }
        }

        const colorUpdate = extractColorUpdateCandidate(row, sourceUrl);
        if (!colorUpdate) continue;
        if (stageMode) continue;
        colorCandidates.push(colorUpdate);
        if (colorCandidates.length >= COLOR_FLUSH_SIZE) {
          colorUpdatesApplied += await applyColorUpdates(
            supabase,
            config.productsTable,
            colorCandidates.splice(0, colorCandidates.length)
          );
        }
      }
    }
    normalized += products.length;

    // Fields managed by humans/editorial — never overwrite on re-sync.
    // Never let a partial feed downgrade live approval/active flags (overnight wipe class of bugs).
    const CURATED_FIELDS = new Set([
      'tags',
      'collection_slugs',
      'editorial_categories',
      'matching_set_id',
      'added_at',
      'approved',
      'is_active',
      'is_displayable',
    ]);

    for (const batch of chunk(products, config.batchSize)) {
      if (stageMode) {
        const staged = await withRetry(async () =>
          staging.stageProductBatch(supabase, stagingSession.session_id, batch)
        );
        upserted += staged;
        stats.updatedProducts += staged;
        await staging.bumpStagingSession(supabase, stagingSession.session_id, {
          files_processed: filesProcessed,
          total_catalog_files: totalCatalogFiles,
          row_count: upserted,
        });
        continue;
      }

      const productIds = batch.map((p) => p.product_id);

      // Find which products already exist (to preserve their curated fields)
      const { data: existing } = await withRetry(async () => {
        const result = await supabase
          .from(config.productsTable)
          .select('product_id')
          .in('product_id', productIds);
        if (result.error) throw result.error;
        return result;
      });

      const existingIds = new Set((existing || []).map((r) => r.product_id));

      // New products: insert with all fields including computed approved/tags
      const newProducts = batch.filter((p) => !existingIds.has(p.product_id));
      if (newProducts.length > 0) {
        await withRetry(async () => {
          const { error } = await supabase.from(config.productsTable).insert(newProducts);
          if (error) throw error;
        });
        upserted += newProducts.length;
        stats.newProducts += newProducts.length;
        stats.nowApproved += newProducts.filter((p) => p.approved === 'yes').length;
      }

      // Existing products: upsert feed-driven fields (including approved / NFP / is_active)
      const updateProducts = batch
        .filter((p) => existingIds.has(p.product_id))
        .map((p) => Object.fromEntries(Object.entries(p).filter(([k]) => !CURATED_FIELDS.has(k))));

      if (updateProducts.length > 0) {
        await withRetry(async () => {
          const { error } = await supabase
            .from(config.productsTable)
            .upsert(updateProducts, { onConflict: 'product_id', ignoreDuplicates: false });
          if (error) throw error;
        });
        upserted += updateProducts.length;
        stats.updatedProducts += updateProducts.length;
        stats.nowApproved += updateProducts.filter((p) => p.approved === 'yes').length;
      }
    }

    if (colorCandidates.length && !stageMode) {
      colorUpdatesApplied += await applyColorUpdates(
        supabase,
        config.productsTable,
        colorCandidates
      );
    }
    if (compositionCandidates.length && !stageMode) {
      compositionUpdatesApplied += await applyCompositionUpdates(
        supabase,
        config.productsTable,
        compositionCandidates
      );
    }
  }

  if (config.hasFtp) {
    try {
      const maxUpserts = Number(process.env.FEED_SYNC_MAX_UPSERTS || 0);
      let lastFtpFile = null;
      const ftpResult = await fetchRakutenFTP(config, errors, async ({ rows, filename }) => {
        // Count each remote file once (onFile is invoked per flush wave).
        if (filename !== lastFtpFile) {
          filesProcessed += 1;
          lastFtpFile = filename;
          console.log(`FTP: processing file ${filesProcessed}/${config.fileLimit || 'all'}: ${filename}`);
        }
        try {
          await processRows(rows, `ftp://${config.ftpHost}/${filename}`);
        } catch (err) {
          const message = err instanceof Error ? err.message : typeof err === 'object' ? JSON.stringify(err) : String(err);
          errors.push({ feedUrl: filename, message });
        }
        if (maxUpserts > 0 && upserted >= maxUpserts) {
          const stop = new Error(`FEED_SYNC_MAX_UPSERTS_REACHED:${maxUpserts}`);
          stop.code = 'FEED_SYNC_MAX_UPSERTS_REACHED';
          throw stop;
        }
      });
      totalCatalogFiles = ftpResult.totalCatalogFiles;
      if (ftpResult.stoppedEarly) {
        console.warn(`[stage] stopped early after FEED_SYNC_MAX_UPSERTS=${process.env.FEED_SYNC_MAX_UPSERTS} (upserted=${upserted})`);
      }
    } catch (err) {
      if (err?.code === 'FEED_SYNC_MAX_UPSERTS_REACHED' || /FEED_SYNC_MAX_UPSERTS_REACHED/.test(String(err?.message || err))) {
        // Fallback if an older fetch path still throws out of the FTP helper.
        console.warn(`[stage] stopped early after FEED_SYNC_MAX_UPSERTS=${process.env.FEED_SYNC_MAX_UPSERTS} (upserted=${upserted})`);
        if (err.totalCatalogFiles != null) totalCatalogFiles = Number(err.totalCatalogFiles);
      } else {
        errors.push({ feedUrl: `ftp://${config.ftpHost}`, message: err.message });
      }
    }
  }

  for (const feedUrl of config.feedUrls) {
    try {
      const rows = await fetchRakutenFeed(feedUrl, config);
      await processRows(rows, feedUrl);
    } catch (error) {
      errors.push({ feedUrl, message: error.message });
    }
  }

  let inactiveMarked = 0;
  if (config.markInactive && !stageMode) {
    let cycleStartedAt = syncStartedAt;
    try {
      const { data: cycleRow } = await supabase
        .from('system_status')
        .select('value_json')
        .eq('key', 'rakuten_feed_sync_cycle')
        .maybeSingle();
      if (cycleRow?.value_json?.cycleStartedAt) {
        cycleStartedAt = cycleRow.value_json.cycleStartedAt;
      }
    } catch {
      // fall back to this run's syncStartedAt
    }
    inactiveMarked = await markInactiveProducts(supabase, config, cycleStartedAt);
  }

  let soldOutDeactivated = 0;
  if (!stageMode) {
    try {
      soldOutDeactivated = await deactivateUnavailableProducts(supabase, config.productsTable);
    } catch (err) {
      errors.push({ feedUrl: 'deactivate_unavailable', message: err.message });
    }
  }

  let stockStatusBackfilled = 0;
  if (!stageMode) {
    try {
      stockStatusBackfilled = await backfillStockStatus(supabase, config.productsTable);
    } catch (err) {
      errors.push({ feedUrl: 'backfill_stock_status', message: err.message });
    }
  }

  if (stageMode && stagingSession?.session_id) {
    try {
      const cycleComplete =
        Number(totalCatalogFiles || 0) > 0 &&
        Number(config.fileOffset || 0) + Number(filesProcessed || 0) >= Number(totalCatalogFiles || 0);
      const { count: exactStaged } = await supabase
        .from('feed_staged_rows')
        .select('product_id', { count: 'exact', head: true })
        .eq('session_id', stagingSession.session_id);
      await staging.bumpStagingSession(supabase, stagingSession.session_id, {
        files_processed: filesProcessed,
        total_catalog_files: totalCatalogFiles,
        file_offset: config.fileOffset,
        row_count: Number(exactStaged || upserted),
        cycle_complete: cycleComplete,
        status: cycleComplete ? 'complete' : 'open',
        completed_at: cycleComplete ? new Date().toISOString() : null,
        meta: {
          syncStartedAt,
          upserted,
          filesProcessed,
          totalCatalogFiles,
          rejected: skippedOutOfScope,
          missingComposition: stats.missingComposition,
          totalProcessed: stats.totalProcessed,
          ingestMode: 'stage',
          additiveOnly: true,
          errors: errors.slice(0, 20),
        },
      });
    } catch (stageErr) {
      errors.push({ feedUrl: 'staging_session', message: stageErr.message });
    }
  }

  try {
    await supabase.from('system_status').upsert({
      key: 'rakuten_feed_sync',
      value_json: {
        syncStartedAt,
        fetched,
        normalized,
        skippedOutOfScope,
        upserted,
        colorUpdatesApplied,
        inactiveMarked,
        soldOutDeactivated,
        stockStatusBackfilled,
        errorCount: errors.length,
        fileOffset: config.fileOffset,
        fileLimit: config.fileLimit,
        filesProcessed,
        totalCatalogFiles,
        markInactive: config.markInactive,
        stats,
      },
      updated_at: new Date().toISOString(),
    });
  } catch (statusErr) {
    errors.push({ feedUrl: 'system_status', message: statusErr.message });
  }

  // Durable per-run audit (ingest may delay products; never erase via silent failures).
  try {
    await supabase.from('catalog_sync_runs').insert({
      started_at: syncStartedAt,
      ended_at: new Date().toISOString(),
      merchant: 'rakuten',
      products_received: fetched,
      inserted_count: stats.newProducts || 0,
      updated_count: Math.max(0, (upserted || 0) - (stats.newProducts || 0)),
      deactivated_count: (inactiveMarked || 0) + (soldOutDeactivated || 0),
      rejected_count: skippedOutOfScope || 0,
      failure_reason: errors.length ? errors.map((e) => e.message || String(e)).join(' | ').slice(0, 2000) : null,
      meta: {
        fileOffset: config.fileOffset,
        fileLimit: config.fileLimit,
        filesProcessed,
        totalCatalogFiles,
        markInactive: config.markInactive,
        colorUpdatesApplied,
        stockStatusBackfilled,
        stats,
      },
    });
  } catch (auditErr) {
    errors.push({ feedUrl: 'catalog_sync_runs', message: auditErr.message });
  }

  stats.errors = errors.map((e) => e.message || String(e));
  try {
    await supabase.from('sync_logs').insert({
      run_at: new Date().toISOString(),
      stats,
      status: errors.length > 0 ? 'partial' : 'success',
      source: config.ftpDirFilter?.length ? `rakuten:${config.ftpDirFilter.join(',')}` : 'rakuten',
    });
  } catch (logErr) {
    errors.push({ feedUrl: 'sync_logs', message: logErr.message });
  }

  console.log('Sync complete:', JSON.stringify({ ...stats, upserted, inactiveMarked, errors: errors.length }, null, 2));

  let materializedViewsRefreshed = [];
  if (!stageMode && upserted > 0 && String(process.env.CATALOG_MV_REFRESH_AFTER_SYNC || '1') === '1') {
    try {
      materializedViewsRefreshed = await refreshCatalogMaterializedViews();
    } catch (mvErr) {
      console.warn('Catalog MV refresh deferred:', mvErr.message || mvErr);
      errors.push({ feedUrl: 'mv_refresh', message: mvErr.message || String(mvErr) });
    }
  }

  // Always sync designers after a successful product write (all regions).
  // Prefer slug-scoped updates from this run — full-table RPC times out on large catalogs.
  let designersSynced = 0;
  if (!stageMode && (upserted > 0 || config.markInactive)) {
    try {
      console.log('Running designer sync (scoped)...');
      const { data: recent, error: recentErr } = await supabase
        .from(config.productsTable)
        .select('brand_slug')
        .gte('updated_at', syncStartedAt)
        .not('brand_slug', 'is', null)
        .limit(5000);
      if (recentErr) throw recentErr;
      const slugs = [...new Set((recent || []).map((r) => String(r.brand_slug || '').trim()).filter(Boolean))];
      if (slugs.length) {
        const now = new Date().toISOString();
        const minProducts = Number(process.env.DESIGNER_LIVE_MIN_PRODUCTS || 2);
        for (const slug of slugs) {
          const { count, error: countErr } = await supabase
            .from('live_products_apparel')
            .select('id', { count: 'exact', head: true })
            .eq('brand_slug', slug);
          if (countErr) continue;
          const cnt = Number(count || 0);
          if (cnt < minProducts) continue;
          const { error: upErr } = await supabase
            .from('designers')
            .update({
              is_live: true,
              status: 'Approved',
              product_count: cnt,
              last_synced_at: now,
            })
            .eq('slug', slug);
          if (!upErr) designersSynced += 1;
        }
      }
      console.log('Designer sync complete:', designersSynced, 'slugs', slugs.length);
    } catch (designerSyncErr) {
      const message =
        designerSyncErr instanceof Error
          ? designerSyncErr.message
          : typeof designerSyncErr === 'object' && designerSyncErr && 'message' in designerSyncErr
            ? String(designerSyncErr.message)
            : JSON.stringify(designerSyncErr);
      console.warn('Designer sync deferred:', message);
      errors.push({ feedUrl: 'designer_sync', message });
    }
  }

  return {
    ok: errors.length === 0,
    syncStartedAt,
    fetched,
    normalized,
    skippedOutOfScope,
    upserted,
    colorUpdatesApplied,
    inactiveMarked,
    soldOutDeactivated,
    stockStatusBackfilled,
    fileOffset: config.fileOffset,
    fileLimit: config.fileLimit,
    filesProcessed,
    totalCatalogFiles,
    markInactive: config.markInactive,
    designersSynced,
    materializedViewsRefreshed,
    ingestMode: ingestGate.mode,
    stagingSessionId: stagingSession?.session_id || null,
    stats,
    errors,
  };
}

function normalizeFtpSecret(value) {
  if (value == null) return '';
  return String(value)
    .replace(/^\uFEFF/, '')
    .trim()
    .replace(/^["']|["']$/g, '');
}

function readConfig(options) {
  const supabaseUrl =
    options.supabaseUrl ||
    process.env.SUPABASE_URL ||
    process.env.VITE_SUPABASE_URL ||
    process.env.NEXT_PUBLIC_SUPABASE_URL;
  const supabaseServiceRoleKey = options.supabaseServiceRoleKey || process.env.SUPABASE_SERVICE_ROLE_KEY;
  const feedUrls = parseList(options.feedUrls || process.env.RAKUTEN_FEED_URLS || process.env.RAKUTEN_FEED_URL);
  const ftpUsername = normalizeFtpSecret(
    options.ftpUsername || process.env.RAKUTEN_FTP_USERNAME || process.env.RAKUTEN_FTP_USER
  );
  const ftpPassword = normalizeFtpSecret(options.ftpPassword || process.env.RAKUTEN_FTP_PASSWORD);
  const ftpHost =
    normalizeFtpSecret(options.ftpHost || process.env.RAKUTEN_FTP_HOST) || 'aftp.linksynergy.com';
  const skipFtp = options.skipFtp === true;
  const hasFtp = !skipFtp && Boolean(ftpUsername && ftpPassword);
  // HARD DEFAULT OFF: production catalog must never be shrunk by a partial/failed feed.
  // Opt in only with CATALOG_ALLOW_MARK_INACTIVE=1 AND an explicit markInactive:true.
  const allowMarkInactive = String(process.env.CATALOG_ALLOW_MARK_INACTIVE || '') === '1';
  const markInactive =
    allowMarkInactive &&
    (options.markInactive === true ||
      (options.markInactive !== false && options.fileLimit == null && !skipFtp));
  if (!supabaseUrl) throw new Error('SUPABASE_URL is required.');
  if (!supabaseServiceRoleKey) throw new Error('SUPABASE_SERVICE_ROLE_KEY is required.');
  if (feedUrls.length === 0 && !hasFtp && !markInactive) {
    throw new Error('RAKUTEN_FEED_URLS or RAKUTEN_FTP_USERNAME + RAKUTEN_FTP_PASSWORD are required.');
  }
  if (hasFtp) {
    // Never log secrets — lengths only. Truncated passwords (common Vercel paste bug) fail Rakuten with 530.
    console.log(
      `[ftp] host=${ftpHost} userLen=${ftpUsername.length} passwordLen=${ftpPassword.length}` +
        (ftpPassword.length > 0 && ftpPassword.length < 16
          ? ' WARNING:password_looks_truncated'
          : '')
    );
  }
  return {
    supabaseUrl,
    supabaseServiceRoleKey,
    feedUrls,
    ftpUsername,
    ftpPassword,
    ftpHost,
    hasFtp,
    skipFtp,
    productsTable: options.productsTable || process.env.SUPABASE_PRODUCTS_TABLE || 'products',
    batchSize: Number(options.batchSize || process.env.FEED_SYNC_BATCH_SIZE || DEFAULT_BATCH_SIZE),
    fileLimit: options.fileLimit != null ? Number(options.fileLimit) : null,
    fileOffset: Number(options.fileOffset || 0),
    markInactive,
    ftpDirFilter: options.ftpDirFilter != null
      ? (Array.isArray(options.ftpDirFilter) ? options.ftpDirFilter : parseList(options.ftpDirFilter))
      : (parseList(process.env.RAKUTEN_FTP_DIR_FILTER).length ? parseList(process.env.RAKUTEN_FTP_DIR_FILTER) : null),
    timeoutMs: Number(options.timeoutMs || process.env.FEED_SYNC_TIMEOUT_MS || 120000),
    authHeader: options.authHeader || process.env.RAKUTEN_AUTH_HEADER,
    bearerToken: options.bearerToken || process.env.RAKUTEN_ACCESS_TOKEN,
    username: options.username || process.env.RAKUTEN_USERNAME,
    password: options.password || process.env.RAKUTEN_PASSWORD
  };
}

async function fetchRakutenFTP(config, errors, onFile) {
  const ftpAccess = {
    host: config.ftpHost,
    port: 21,
    user: config.ftpUsername,
    password: config.ftpPassword,
    secure: false,
  };

  // Helper: create a fresh connected FTP client (IPv4 + passive — required on serverless).
  async function connect() {
    const c = new FtpClient(90000);
    c.ftp.verbose = false;
    c.ftp.passive = true;
    try {
      c.ftp.ipFamily = 4;
    } catch {
      /* older basic-ftp */
    }
    await c.access(ftpAccess);
    return c;
  }

  async function listPath(path, attempts = 3) {
    let lastErr;
    for (let i = 0; i < attempts; i++) {
      let c;
      try {
        c = await connect();
        const listed = await c.list(path);
        c.close();
        return listed;
      } catch (err) {
        lastErr = err;
        try {
          c?.close();
        } catch {
          /* ignore */
        }
        const waitMs = 750 * (i + 1);
        console.warn(`FTP list retry ${i + 1}/${attempts} for ${path}:`, err?.message || err);
        await new Promise((r) => setTimeout(r, waitMs));
      }
    }
    throw lastErr || new Error(`Could not list ${path}`);
  }

  function isCatalogFile(f) {
    const n = String(f?.name || '').toLowerCase();
    if (!n) return false;
    if (n.includes('delta') || n.includes('template')) return false;
    return /\.(xml|txt|tsv|csv)(\.gz)?$/.test(n);
  }

  // Step 1: resilient catalog discovery.
  // Prefer top-level merchant files (MID_*.xml.gz) — one LIST discovers all new MIDs.
  // Then optionally enrich from MID subdirectories with fresh connections (never fail the run).
  let catalogFiles = [];
  {
    const allFiles = [];
    const dirFilter = config.ftpDirFilter?.length ? config.ftpDirFilter : null;
    let topList = [];
    try {
      topList = await listPath('.');
    } catch (err) {
      const message = err?.message || String(err);
      errors.push({ feedUrl: `ftp://${config.ftpHost}`, message });
      throw new Error(`450 Could not list: ${message}`);
    }

    const midDirs = [];
    for (const entry of topList) {
      if (entry.type === 2 && /^\d+$/.test(String(entry.name || ''))) {
        midDirs.push(String(entry.name));
      } else if (entry.type !== 2) {
        allFiles.push(entry);
      }
    }

    const dirsToScan = dirFilter
      ? dirFilter.filter((mid) => midDirs.includes(mid) || true)
      : midDirs;

    for (const mid of dirsToScan) {
      if (dirFilter && !dirFilter.includes(mid)) continue;
      try {
        const subList = await listPath(mid, 2);
        for (const f of subList) allFiles.push({ ...f, dir: mid });
      } catch (err) {
        console.warn(`FTP: could not list ${mid}:`, err?.message || err);
        errors.push({ feedUrl: `ftp://${config.ftpHost}/${mid}`, message: err?.message || String(err) });
      }
    }

    catalogFiles = allFiles.filter((f) => {
      if (!isCatalogFile(f)) return false;
      const n = String(f.name || '').toLowerCase();
      if (dirFilter && f.dir && !dirFilter.includes(f.dir)) return false;
      if (dirFilter && !f.dir) {
        return dirFilter.some((mid) => n.startsWith(String(mid)));
      }
      return true;
    });

    // Stable order so chunk offsets advance deterministically across runs.
    catalogFiles.sort((a, b) => {
      const ak = `${a.dir || ''}/${a.name}`.toLowerCase();
      const bk = `${b.dir || ''}/${b.name}`.toLowerCase();
      return ak.localeCompare(bk);
    });

    if (catalogFiles.length === 0) {
      throw new Error('FTP listing returned zero catalog files');
    }
  }

  const MAX_FILE_BYTES = 1000 * 1024 * 1024; // 1 GB compressed
  const totalCatalogFiles = catalogFiles.length;
  const sliceStart = config.fileOffset || 0;
  const sliceEnd = config.fileLimit != null ? sliceStart + config.fileLimit : totalCatalogFiles;
  catalogFiles = catalogFiles.slice(sliceStart, sliceEnd);
  console.log(`FTP: found ${totalCatalogFiles} catalog files, processing ${catalogFiles.length} (offset ${sliceStart})`);

  // Step 2: download and process each file with its own fresh FTP connection
  for (const file of catalogFiles) {
    if (file.size > MAX_FILE_BYTES) {
      console.warn(`FTP: skipping ${file.name} (${Math.round(file.size / 1024 / 1024)}MB > 1GB limit)`);
      continue;
    }
    if (SKIP_FTP_FILES.has(String(file.name || '').toLowerCase())) {
      console.warn(`FTP: skipping ${file.name} (known OOM / denylist)`);
      continue;
    }
    const remotePath = file.dir ? `${file.dir}/${file.name}` : file.name;
    let client;
    try {
      client = await connect();
      const chunks = [];
      const stream = new PassThrough();
      stream.on('data', (chunk) => chunks.push(chunk));
      await client.downloadTo(stream, remotePath);
      client.close();
      client = null;

      let compressedBuffer = Buffer.concat(chunks);
      chunks.length = 0;
      const isGzip = file.name.endsWith('.gz');
      const isXml = file.name.replace(/\.gz$/i, '').endsWith('.xml');
      let rows = [];
      let flushedRows = 0;

      async function flushRows(force = false) {
        if (!rows.length) return;
        if (!force && rows.length < FTP_ROW_FLUSH_SIZE) return;
        const batch = rows;
        rows = [];
        flushedRows += batch.length;
        console.log(`FTP: ${remotePath} — flushing ${batch.length} rows (total ${flushedRows})`);
        await onFile({ filename: remotePath, rows: batch });
      }

      if (isGzip && compressedBuffer.length > 5_000_000) {
        // Stream-decompress large files to avoid V8 string length limit
        // Parse incrementally — never join all chunks into one string
        await new Promise((resolve, reject) => {
          // tail = unprocessed text carried between gunzip data events.
          // NEVER grows beyond ~2 product blocks (~50KB) because we drain it each event.
          let tail = '';
          const delimitedParts = [];
          let chain = Promise.resolve();

          // Find next <product> or <product SPACE tag (not <productImage> etc.)
          function nextProductOpen(text, from) {
            let pos = from;
            while (pos < text.length) {
              const i = text.indexOf('<product', pos);
              if (i === -1) return -1;
              const c = text[i + 8]; // char after '<product'
              if (c === '>' || c === ' ' || c === '\n' || c === '\t' || c === '\r') return i;
              pos = i + 1;
            }
            return -1;
          }

          function drainProducts(text) {
            // Extract and process all complete outer <product>...</product> blocks.
            // Returns any incomplete trailing fragment.
            let start = 0;
            while (start < text.length) {
              const open = nextProductOpen(text, start);
              if (open === -1) return ''; // nothing left to keep
              let depth = 1, s = open + 1, close = -1;
              while (depth > 0 && s < text.length) {
                const no = nextProductOpen(text, s);
                const nc = text.indexOf('</product>', s);
                if (nc === -1) break;
                if (no !== -1 && no < nc) {
                  depth++;
                  s = no + 1;
                } else {
                  depth--;
                  if (depth === 0) close = nc;
                  s = nc + 1;
                }
              }
              if (close === -1) {
                // Incomplete product — keep from 'open' as tail
                return text.slice(open);
              }
              const endIdx = close + '</product>'.length;
              const xmlChunk = text.slice(open, endIdx);
              start = endIdx;

              // Fast pre-filter: skip without XML parse when clearly out of consumer scope
              const cl = xmlChunk.toLowerCase();
              if (!passesRakutenXmlPrefilter(cl)) continue;

              try {
                const p = (makeXMLParser().parse(xmlChunk)).product;
                if (p && p['@_product_id']) rows.push(flattenRakutenXMLProduct(p));
              } catch { /* skip malformed */ }
            }
            return '';
          }

          const readable = Readable.from(compressedBuffer);
          const gunzip = createGunzip();
          readable.pipe(gunzip);

          gunzip.on('data', (rawChunk) => {
            if (!isXml) { delimitedParts.push(rawChunk.toString('utf-8')); return; }
            // Prepend only the small tail, not the whole history
            const text = tail + rawChunk.toString('utf-8');
            tail = drainProducts(text);
            // Safety: if tail grows beyond 50MB a product is malformed — discard it
            if (tail.length > 50_000_000) tail = '';
            if (rows.length >= FTP_ROW_FLUSH_SIZE) {
              gunzip.pause();
              chain = chain
                .then(() => flushRows(true))
                .then(() => gunzip.resume())
                .catch(reject);
            }
          });

          gunzip.on('end', () => {
            chain
              .then(async () => {
                if (!isXml) {
                  rows = parseDelimited(delimitedParts.join(''));
                  delimitedParts.length = 0;
                } else if (tail) {
                  drainProducts(tail + '</product>'); // attempt flush
                  tail = '';
                }
                // Drop compressed bytes before DB work
                compressedBuffer = Buffer.alloc(0);
                await flushRows(true);
                resolve();
              })
              .catch(reject);
          });
          gunzip.on('error', reject);
        });
      } else {
        const body = isGzip ? gunzipSync(compressedBuffer) : compressedBuffer;
        compressedBuffer = Buffer.alloc(0);
        rows = isXml ? parseXML(body.toString('utf-8')) : parseDelimited(body.toString('utf-8'));
        console.log(`FTP: ${remotePath} — ${rows.length} rows`);
        // Flush in waves so upsert path cannot hold the whole file in memory twice.
        while (rows.length) {
          const wave = rows.splice(0, FTP_ROW_FLUSH_SIZE);
          flushedRows += wave.length;
          console.log(`FTP: ${remotePath} — flushing ${wave.length} rows (total ${flushedRows})`);
          await onFile({ filename: remotePath, rows: wave });
        }
      }

      if (flushedRows === 0 && rows.length === 0) {
        console.log(`FTP: ${remotePath} — 0 rows after filter`);
      }
    } catch (err) {
      if (err?.code === 'FEED_SYNC_MAX_UPSERTS_REACHED' || /FEED_SYNC_MAX_UPSERTS_REACHED/.test(String(err?.message || ''))) {
        if (client) { try { client.close(); } catch {} }
        return { totalCatalogFiles, stoppedEarly: true };
      }
      const message = err instanceof Error
        ? err.message
        : typeof err === 'object' && err !== null
          ? JSON.stringify(err)
          : String(err);
      console.warn(`FTP file error ${file.name}: ${message}`);
      errors.push({ feedUrl: remotePath, message });
      if (client) { try { client.close(); } catch {} }
    }
  }

  return { totalCatalogFiles, stoppedEarly: false };
}

async function fetchRakutenFeed(feedUrl, config) {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), config.timeoutMs);
  try {
    const headers = {};
    if (config.authHeader) headers.Authorization = config.authHeader;
    if (config.bearerToken) headers.Authorization = `Bearer ${config.bearerToken}`;
    if (config.username && config.password) {
      headers.Authorization = `Basic ${Buffer.from(`${config.username}:${config.password}`).toString('base64')}`;
    }
    const response = await fetch(feedUrl, { headers, signal: controller.signal });
    if (!response.ok) throw new Error(`Rakuten feed failed ${response.status} ${response.statusText}`);
    const body = await response.text();
    return parseFeed(body, response.headers.get('content-type') || feedUrl);
  } finally {
    clearTimeout(timeout);
  }
}

function flattenRakutenXMLProduct(p) {
  if (!p || typeof p !== 'object') return p;
  const str = (v) => {
    if (v === undefined || v === null) return undefined;
    return String(v)
      .replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>')
      .replace(/&quot;/g, '"').replace(/&apos;/g, "'").trim();
  };
  const attr = p.attributeClass || {};
  const secondary = str(p.category?.secondary);
  const mid = str(p['@_mid'] || (String(p['@_product_id'] || '').slice(0, 5)));
  return {
    product_id:      str(p['@_product_id'] || p.product_id),
    name:            str(p['@_name'] || p.name),
    sku:             str(p['@_sku_number'] || p.sku_number || p.sku),
    upc:             str(p.upc),
    brand:           str(p.brand || p.manufacturer_name || p['@_manufacturer_name']),
    manufacturer:    str(p.manufacturer_name || p['@_manufacturer_name']),
    category:        str(p.category?.primary || p.category),
    secondary_category: secondary,
    merchant_category: secondary,
    product_url:     str(p.URL?.product || p.url),
    image_url:       str(p.URL?.productImage || p.image_url),
    description:     str(p.description?.long || p.description?.short || p.description),
    short_desc:      str(p.description?.short),
    // Retail feeds often include both values. The sale/current amount must be
    // the displayed price; retail is the crossed-out original price.
    price:           str(p.price?.sale || p.price?.retail || p.price),
    sale_price:      str(p.price?.sale),
    retail_price:    str(p.price?.retail),
    discount_amount: str(p.price?.discount || p.discount_amount || p.savings),
    discount_percent: str(p.price?.percent_off || p.discount_percent),
    currency:        str(p.price?.['@_currency'] || p.price?.currency || 'USD'),
    availability:    str(p.shipping?.availability),
    mid,
    advertiser_id:   mid,
    merchant_id:     mid,
    advertiser_name: isBloomingdalesRetailerMid(mid) ? "Bloomingdale's" : str(p.advertiser_name || p.merchant_name),
    merchant_name:   isBloomingdalesRetailerMid(mid) ? "Bloomingdale's" : str(p.merchant_name),
    // Mytheresa attributeClass fields
    material:        str(attr.Material || attr.material),
    gender:          str(attr.Gender || attr.gender),
    product_type:    str(attr.Product_Type || attr.product_type),
    color:           str(attr.Color || attr.color),
    size:            str(attr.Size || attr.size),
  };
}

function makeXMLParser() {
  return new XMLParser({
    ignoreAttributes: false,
    attributeNamePrefix: '@_',
    isArray: () => false,
    htmlEntities: false,
    processEntities: false,
    parseAttributeValue: false,
  });
}

function nextProdOpen(text, from) {
  let pos = from;
  while (pos < text.length) {
    const i = text.indexOf('<product', pos);
    if (i === -1) return -1;
    const c = text[i + 8];
    if (c === '>' || c === ' ' || c === '\n' || c === '\t' || c === '\r') return i;
    pos = i + 1;
  }
  return -1;
}

function parseXMLChunked(body) {
  const parser = makeXMLParser();
  const results = [];
  let pos = 0;
  while (pos < body.length) {
    const open = nextProdOpen(body, pos);
    if (open === -1) break;
    let depth = 1;
    let search = open + 1;
    let close = -1;
    while (depth > 0 && search < body.length) {
      const nextOpen  = nextProdOpen(body, search);
      const nextClose = body.indexOf('</product>', search);
      if (nextClose === -1) break;
      if (nextOpen !== -1 && nextOpen < nextClose) {
        depth++;
        search = nextOpen + 1;
      } else {
        depth--;
        if (depth === 0) close = nextClose;
        search = nextClose + 1;
      }
    }
    if (close === -1) { pos = open + 1; continue; }
    const chunk = body.slice(open, close + '</product>'.length);
    pos = close + '</product>'.length;
    try {
      const parsed = parser.parse(chunk);
      const p = parsed.product || parsed;
      if (p && p['@_product_id']) results.push(flattenRakutenXMLProduct(p));
    } catch { /* skip malformed chunk */ }
  }
  return results;
}

function parseFeed(body, hint) {
  const trimmed = body.trim();
  if (!trimmed) return [];
  if (hint.includes('json') || trimmed.startsWith('{') || trimmed.startsWith('[')) {
    const parsed = JSON.parse(trimmed);
    if (Array.isArray(parsed)) return parsed;
    return parsed.products || parsed.items || parsed.data || [];
  }
  if (trimmed.startsWith('<')) {
    const parser = makeXMLParser();
    const parsed = parser.parse(trimmed);
    const root = parsed.merchandiser || parsed.products || parsed.items || parsed.rss?.channel || parsed.feed || {};
    const raw = root.product || root.item || root.entry || [];
    const candidates = Array.isArray(raw) ? raw : (raw && typeof raw === 'object' ? [raw] : []);
    return candidates.map(flattenRakutenXMLProduct);
  }
  return parseDelimited(trimmed);
}

function parseXML(body) {
  return parseFeed(body, 'xml');
}

function parseDelimited(body) {
  const lines = body.split(/\r?\n/).filter(Boolean);
  if (lines.length < 2) return [];
  const delimiter = lines[0].includes('\t') ? '\t' : ',';
  const headers = splitLine(lines[0], delimiter).map(normalizeKey);
  return lines.slice(1).map((line) => {
    const values = splitLine(line, delimiter);
    return Object.fromEntries(headers.map((header, index) => [header, values[index] || '']));
  });
}

function splitLine(line, delimiter) {
  if (delimiter === '\t') return line.split('\t');
  return line.match(/("[^"]*"|[^,]+)/g)?.map((value) => value.replace(/^"|"$/g, '').trim()) || [];
}

function extractGarmentType(productName, category) {
  const gt = classifyGarment(category || '', productName || '');
  return gt && gt !== 'needs_review' ? gt : null;
}

function extractRowGateFields(row, feedUrl) {
  const rawName = cleanText(first(row, ['name', 'title', 'product_name', 'productname', 'description']));
  const rawCompositionRaw = cleanText(first(row, [
    'composition', 'material', 'fabric', 'materials',
    'material_description', 'fabric_description',
    'g_material', 'google_custom_label_1', 'custom_label_1',
  ]));
  const rawComposition = stripCompositionPrefix(rawCompositionRaw);
  const composition = isValidCompositionString(rawComposition) ? rawComposition : undefined;
  const categorySource = [
    cleanText(first(row, ['category', 'product_type', 'google_product_category', 'merchant_category', 'product_category', 'department'])),
    cleanText(first(row, ['secondary_category', 'product_type'])),
  ].filter(Boolean).join(' ');
  const category = normalizeCategory(categorySource, rawName);
  const name = refineProductTitle(rawName, category, composition);
  const price = cleanText(first(row, ['sale_price', 'current_price', 'price', 'retail_price']));
  const imageUrl = first(row, ['image_url', 'image', 'image_link', 'thumbnail', 'picture', 'additional_image_link']);
  const url = first(row, ['link', 'url', 'product_url', 'click_url', 'buy_url']);
  const gender = cleanText(first(row, ['gender', 'sex', 'target_gender']));
  return { category, name, composition, imageUrl, price, url, gender };
}

async function applyColorUpdates(supabase, table, updates) {
  const byId = new Map();
  for (const update of updates) {
    if (!update?.product_id || !update?.color) continue;
    if (!byId.has(update.product_id)) byId.set(update.product_id, update.color);
  }
  if (!byId.size) return 0;

  let applied = 0;
  const ids = [...byId.keys()];
  const LOOKUP_CHUNK = 200;
  const UPDATE_CONCURRENCY = 25;

  for (let i = 0; i < ids.length; i += LOOKUP_CHUNK) {
    const idChunk = ids.slice(i, i + LOOKUP_CHUNK);
    const { data: targets, error } = await withRetry(async () => {
      const result = await supabase
        .from(table)
        .select('product_id')
        .in('product_id', idChunk)
        .is('color', null);
      if (result.error) throw result.error;
      return result;
    });
    if (!targets?.length) continue;

    for (let j = 0; j < targets.length; j += UPDATE_CONCURRENCY) {
      const updateBatch = targets.slice(j, j + UPDATE_CONCURRENCY);
      const counts = await Promise.all(
        updateBatch.map(({ product_id }) =>
          withRetry(async () => {
            const color = byId.get(product_id);
            const { data, error } = await supabase
              .from(table)
              .update({ color, color_normalized: color })
              .eq('product_id', product_id)
              .is('color', null)
              .select('product_id');
            if (error) throw error;
            return data?.length || 0;
          })
        )
      );
      applied += counts.reduce((sum, n) => sum + n, 0);
    }
  }

  return applied;
}

function extractCompositionFromRow(row) {
  const rawCompositionRaw = cleanText(first(row, [
    'composition', 'material', 'fabric', 'materials',
    'material_description', 'fabric_description',
    'g_material', 'google_custom_label_1', 'custom_label_1',
  ]));
  const rawComposition = stripCompositionPrefix(rawCompositionRaw);
  let composition = isValidCompositionString(rawComposition) ? rawComposition : undefined;
  if (!composition) composition = inferCompositionFromRow(row);
  return composition;
}

function extractCompositionUpdateCandidate(row, feedUrl) {
  const productId = first(row, ['product_id', 'productid', 'sku', 'item_id', 'id']);
  if (!productId) return null;

  const composition = extractCompositionFromRow(row);
  if (!composition) return null;

  const mytheresa = isMytheresaRow(row, feedUrl);
  const mid = first(row, ['mid', 'merchant_id', 'advertiser_id', 'network_mid']);
  const { region } = resolveMidRegionCurrency(mid, row, { mytheresa });
  const baseId = String(productId);
  const regionedId = region === 'us' ? baseId : `${baseId}-${region}`;
  const nfp = naturalFiberPercent(composition);
  if (nfp == null) return null;

  return { product_id: regionedId, composition, natural_fiber_percent: nfp };
}

async function applyCompositionUpdates(supabase, table, updates) {
  const byId = new Map();
  for (const update of updates) {
    if (!update?.product_id || !update?.composition) continue;
    byId.set(update.product_id, update);
  }
  if (!byId.size) return 0;

  let applied = 0;
  const ids = [...byId.keys()];
  const LOOKUP_CHUNK = 100;
  const UPDATE_CONCURRENCY = 15;

  for (let i = 0; i < ids.length; i += LOOKUP_CHUNK) {
    const idChunk = ids.slice(i, i + LOOKUP_CHUNK);
    const { data: targets, error } = await withRetry(async () => {
      const result = await supabase
        .from(table)
        .select('product_id, category, name, gender_scope, url, image_url, price, is_active, approved')
        .in('product_id', idChunk);
      if (result.error) throw result.error;
      return result;
    });
    if (error || !targets?.length) continue;

    for (let j = 0; j < targets.length; j += UPDATE_CONCURRENCY) {
      const batch = targets.slice(j, j + UPDATE_CONCURRENCY);
      const counts = await Promise.all(
        batch.map(async (existing) => {
          const patch = byId.get(existing.product_id);
          if (!patch) return 0;
          const category = existing.category || '';
          const name = existing.name || '';
          const genderScope = existing.gender_scope || 'unknown';
          const footwearOk = passesFootwearIngestionGate({
            category,
            name,
            composition: patch.composition,
            imageUrl: existing.image_url,
            price: existing.price,
            url: existing.url,
            gender: '',
          });
          const apparelOk = passesConsumerIngestionGate({
            category,
            name,
            composition: patch.composition,
            imageUrl: existing.image_url,
            price: existing.price,
            url: existing.url,
            gender: '',
          });
          if (!apparelOk && !footwearOk) return 0;

          const consumerApproved = footwearOk && !apparelOk
            ? approvalStatusFootwear(patch.natural_fiber_percent, patch.composition, genderScope)
            : approvalStatus(patch.natural_fiber_percent, patch.composition, category, name, genderScope);
          const consumerLive =
            consumerApproved === 'yes' &&
            (
              footwearOk && !apparelOk
                ? Boolean(patch.composition) || /leather|suede|nubuck/i.test(name)
                : (patch.natural_fiber_percent != null && patch.natural_fiber_percent >= 80)
            );

          return withRetry(async () => {
            const { data, error: updErr } = await supabase
              .from(table)
              .update({
                composition: patch.composition,
                natural_fiber_percent: patch.natural_fiber_percent,
                material_summary: `${patch.natural_fiber_percent}% natural fiber`,
                approved: consumerApproved,
                is_active: consumerLive,
                updated_at: new Date().toISOString(),
                last_seen_at: new Date().toISOString(),
              })
              .eq('product_id', existing.product_id)
              .select('product_id');
            if (updErr) throw updErr;
            return data?.length || 0;
          });
        })
      );
      applied += counts.reduce((sum, n) => sum + n, 0);
    }
  }

  return applied;
}

/** mp.xml stub when composition arrives in a separate cmp file. */
function normalizeRakutenProductStub(row, feedUrl) {
  const productId = first(row, ['product_id', 'productid', 'sku', 'item_id', 'id']);
  const url = first(row, ['link', 'url', 'product_url', 'click_url', 'buy_url']);
  if (!productId || !url) return null;

  const gateFields = extractRowGateFields(row, feedUrl);
  if (consumerExclusionReason(gateFields) !== 'missing_composition') return null;

  const mytheresa = isMytheresaRow(row, feedUrl);
  const bloomingdales = !mytheresa && isBloomingdalesRow(row, feedUrl);
  const mid = first(row, ['mid', 'merchant_id', 'advertiser_id', 'network_mid']);
  const merchantDef = merchantByMid(mid);
  const rawBrand = mytheresa
    ? first(row, MYTHERESA_BRAND_FIELD_KEYS)
    : bloomingdales
      ? first(row, BLOOMINGDALES_BRAND_FIELD_KEYS)
      : first(row, ['brand', 'brand_name', 'designer', 'designer_name', 'manufacturer', 'advertiser_name', 'merchant_name']);
  const registryBrand = merchantDef?.brandSlug
    ? (merchantDef.retailer || merchantDef.label.replace(/\s+(US|UK|Canada)$/i, '').trim())
    : null;
  const brandName = registryBrand ? normalizeBrandName(registryBrand) : normalizeBrandName(cleanText(rawBrand));
  const rawName = cleanText(first(row, ['name', 'title', 'product_name', 'productname', 'description']));
  const categorySource = [
    cleanText(first(row, ['category', 'product_type', 'google_product_category', 'merchant_category', 'product_category', 'department'])),
    cleanText(first(row, ['secondary_category', 'product_type'])),
  ].filter(Boolean).join(' ');
  const category = normalizeCategory(categorySource, rawName);
  const name = refineProductTitle(rawName, category, undefined);
  const { price: resolvedPrice, originalPrice, rawPrice } = resolveOfferPrices(row);
  const imageUrl = first(row, ['image_url', 'image', 'image_link', 'thumbnail', 'picture', 'additional_image_link']);
  const { currency, region, retailerCountry } = resolveMidRegionCurrency(mid, row, { mytheresa });
  const baseId = String(productId);
  const regionedId = region === 'us' ? baseId : `${baseId}-${region}`;
  const now = new Date().toISOString();
  const retailer = mytheresa ? 'MyTheresa' : bloomingdales ? "Bloomingdale's" : normalizeBrandName(cleanText(first(row, ['advertiser_name', 'merchant_name', 'retailer'])));

  return {
    product_id: regionedId,
    brand_name: brandName,
    brand_slug: slugify(brandName),
    name,
    url,
    image_url: imageUrl,
    price: resolvedPrice,
    original_price: originalPrice,
    raw_price: rawPrice,
    currency,
    retailer_country: retailerCountry,
    region,
    retailer,
    category,
    garment_type: extractGarmentType(name, category),
    approved: 'pending',
    is_active: false,
    feed_source: merchantDef?.feedSource || (mytheresa ? 'mytheresa' : bloomingdales ? 'bloomingdales' : 'rakuten'),
    retailer_mid: mid || undefined,
    feed_url: feedUrl,
    last_seen_at: now,
    added_at: now,
    updated_at: now,
    tags: [],
    collection_slugs: [],
    editorial_categories: [],
  };
}

function extractColorUpdateCandidate(row, feedUrl) {
  const productId = first(row, ['product_id', 'productid', 'sku', 'item_id', 'id', 'mid']);
  const url = first(row, ['link', 'url', 'product_url', 'click_url', 'buy_url']);
  if (!productId && !url) return null;

  const mytheresa = isMytheresaRow(row, feedUrl);
  const rawName = cleanText(first(row, ['name', 'title', 'product_name', 'productname', 'description']));
  const rawColor = cleanText(
    first(row, [
      'color', 'colour', 'Color', 'Colour',
      'item_color', 'designer_color', 'product_color',
      'attributeclass_color', 'custom_label_2', 'google_custom_label_2',
    ])
  );
  const colorValue = normalizeColorValue(rawColor) || extractColorFromName(rawName);
  if (!colorValue) return null;

  const mid = first(row, ['mid', 'merchant_id', 'advertiser_id', 'network_mid']);
  const { currency, region, retailerCountry } = resolveMidRegionCurrency(mid, row, { mytheresa });
  const baseId = String(productId || stableId(url));
  const regionedId = region === 'us' ? baseId : `${baseId}-${region}`;

  return { product_id: regionedId, color: colorValue };
}

function normalizeRakutenProduct(row, feedUrl) {
  const productId = first(row, ['product_id', 'productid', 'sku', 'item_id', 'id', 'mid']);
  const url = first(row, ['link', 'url', 'product_url', 'click_url', 'buy_url']);
  if (!productId && !url) return null;

  const mytheresa = isMytheresaRow(row, feedUrl);
  const bloomingdales = !mytheresa && isBloomingdalesRow(row, feedUrl);
  const mid = first(row, ['mid', 'merchant_id', 'advertiser_id', 'network_mid']);
  const merchantDef = merchantByMid(mid);

  const rawBrand = mytheresa
    ? first(row, MYTHERESA_BRAND_FIELD_KEYS)
    : bloomingdales
      ? first(row, BLOOMINGDALES_BRAND_FIELD_KEYS)
      : first(row, ['brand', 'brand_name', 'designer', 'designer_name', 'manufacturer', 'advertiser_name', 'merchant_name']);

  const isVeronicaBeardCanada = isVeronicaBeardCanadaRow(row);
  const registryBrand = merchantDef?.brandSlug
    ? (merchantDef.retailer || merchantDef.label.replace(/\s+(US|UK|Canada)$/i, '').trim())
    : null;
  const brandName = isVeronicaBeardCanada
    ? 'Veronica Beard'
    : registryBrand
      ? normalizeBrandName(registryBrand)
      : normalizeBrandName(cleanText(rawBrand));
  const rawName = cleanText(first(row, ['name', 'title', 'product_name', 'productname', 'description']));

  const rawCompositionRaw = cleanText(first(row, [
    'composition', 'material', 'fabric', 'materials',
    'material_description', 'fabric_description',
    'g_material', 'google_custom_label_1', 'custom_label_1'
  ]));
  // Strip 'Material: ' prefix (Mytheresa format) and truncate at care/origin suffixes
  const rawComposition = stripCompositionPrefix(rawCompositionRaw);
  // Only treat as a real fiber composition if it looks like material data, not a product description.
  let composition = isValidCompositionString(rawComposition) ? rawComposition : undefined;
  if (!composition) {
    composition = inferCompositionFromRow(row);
  }

  const categorySource = [
    cleanText(first(row, ['category', 'product_type', 'google_product_category', 'merchant_category', 'product_category', 'department'])),
    cleanText(first(row, ['secondary_category', 'product_type'])),
  ].filter(Boolean).join(' ');
  const category = normalizeCategory(categorySource, rawName);
  const name = refineProductTitle(rawName, category, composition);
  const { price: resolvedPrice, originalPrice, rawPrice } = resolveOfferPrices(row);
  const imageUrl = first(row, ['image_url', 'image', 'image_link', 'thumbnail', 'picture', 'additional_image_link']);
  const gender = cleanText(first(row, ['gender', 'sex', 'target_gender']));

  // Align with Supabase catalog_consumer_exclusion_reason + catalog_is_womens_apparel_category.
  // Skip insert/upsert entirely — stale rows get is_active=false via markInactiveProducts (no last_seen_at bump).
  // Footwear is allowed through a dedicated natural-leather gate so Natural Shoes rails stay stocked.
  const apparelOk = passesConsumerIngestionGate({ category, name, composition, imageUrl, price: resolvedPrice, url, gender });
  const footwearOk = passesFootwearIngestionGate({ category, name, composition, imageUrl, price: resolvedPrice, url, gender });
  if (!apparelOk && !footwearOk) {
    return null;
  }
  const resolvedCategory = footwearOk && !apparelOk ? 'Footwear' : category;

  const { isAvailable, stockStatus } = resolveAvailability(row);

  const genderScope = classifyGenderScope({ category, name, url, gender });
  const nfp = naturalFiberPercent(composition);
  const consumerApproved = footwearOk && !apparelOk
    ? approvalStatusFootwear(nfp, composition, genderScope)
    : approvalStatus(nfp, composition, resolvedCategory, name, genderScope);
  const consumerLive =
    consumerApproved === 'yes' &&
    isAvailable &&
    (
      footwearOk && !apparelOk
        ? Boolean(composition) || /leather|suede|nubuck/i.test(name)
        : (nfp != null && nfp >= 80 && Boolean(composition))
    );

  const rawPriceVal = rawPrice;
  const { currency, region, retailerCountry } = resolveMidRegionCurrency(mid, row, { mytheresa });
  const retailer = mytheresa
    ? 'Mytheresa'
    : bloomingdales
      ? "Bloomingdale's"
      : normalizeBrandName(cleanText(first(row, ['advertiser_name', 'merchant_name', 'retailer'])));
  const retailerName = retailer || cleanText(first(row, ['merchant_name', 'advertiser_name', 'retailer_name']));
  const sku = first(row, ['sku', 'item_id', 'seller_id', 'merchant_product_id']);
  const upc = first(row, ['upc', 'ean', 'barcode', 'gtin']);
  const rawColor = cleanText(
    first(row, ['color', 'colour', 'Color', 'Colour', 'item_color', 'designer_color', 'product_color'])
  );
  const colorValue = normalizeColorValue(rawColor) || extractColorFromName(name);
  const sizeOptions = parseSizeOptions(row);
  const countryOfOrigin = cleanText(first(row, ['country_of_origin', 'made_in', 'origin_country']));
  const careInstructions = cleanText(first(row, ['care_instructions', 'care', 'care_label']));
  const season = cleanText(first(row, ['season', 'collection', 'season_collection']));
  const materialSummary = nfp != null ? `${nfp}% natural fiber` : (composition ? composition.slice(0, 80) : undefined);
  const now = new Date().toISOString();
  const numericPrice = numberFromPrice(resolvedPrice);
  const numericOriginal = numberFromPrice(originalPrice);
  const discountPercent = numericPrice && numericOriginal && numericOriginal > numericPrice
    ? Math.round(((numericOriginal - numericPrice) / numericOriginal) * 100)
    : undefined;

  // Region-scope the product_id so USD/EUR/GBP variants are separate rows
  const baseId = String(productId || stableId(url));
  const regionedId = region === 'us' ? baseId : `${baseId}-${region}`;

  const normalized = {
    product_id:            regionedId,
    brand_name:            brandName,
    brand_slug:            slugify(brandName),
    name,
    url,
    image_url:             imageUrl,
    price:                 resolvedPrice,
    original_price:        originalPrice,
    raw_price:             rawPriceVal,
    currency,
    retailer_country:      retailerCountry,
    region,
    retailer,
    retailer_name:         retailerName,
    is_sale:               isSale(resolvedPrice, originalPrice, row, { bloomingdales }),
    discount_percent:      discountPercent,
    composition,
    natural_fiber_percent: nfp,
    material_summary:      materialSummary,
    color:                 colorValue,
    color_normalized:      colorValue,
    size_options:          sizeOptions,
    country_of_origin:     countryOfOrigin,
    care_instructions:     careInstructions,
    category:              resolvedCategory,
    garment_type:          extractGarmentType(name, resolvedCategory),
    gender_scope:          genderScope,
    sku,
    upc,
    season,
    approved:              consumerApproved,
    is_active:             consumerLive,
    stock_status:          stockStatus,
    feed_source: mytheresa
      ? 'mytheresa'
      : bloomingdales
        ? 'bloomingdales'
        : merchantDef?.feedSource === 'rakuten' && footwearOk && !apparelOk
          ? 'rakuten_footwear'
          : (merchantDef?.feedSource || (footwearOk && !apparelOk ? 'rakuten_footwear' : 'rakuten')),
    retailer_mid:          mid || undefined,
    feed_url:              feedUrl,
    product_url_us:        region === 'us' ? url : undefined,
    product_url_uk:        region === 'uk' ? url : undefined,
    last_price_check:      now,
    price_changed_at:      discountPercent != null ? now : undefined,
    last_seen_at:          now,
    added_at:              now,
    updated_at:            now,
    tags:                  buildTags(row, composition, resolvedCategory),
    collection_slugs:      buildCollectionSlugs(composition, resolvedCategory),
    editorial_categories:  buildEditorialCategories(composition, category)
  };
  return Object.fromEntries(Object.entries(normalized).filter(([, v]) => v !== undefined && v !== null));
}

async function markInactiveProducts(supabase, config, cycleStartedAt) {
  const feedSources = config.markInactiveSources || ['rakuten', 'mytheresa'];
  // Never wipe the catalog because one incomplete cycle skipped rows.
  // Only deactivate products that have been missing from feeds for this grace window.
  const graceDays = Number(process.env.RAKUTEN_INACTIVE_GRACE_DAYS || 21);
  const maxPerRun = Number(process.env.RAKUTEN_INACTIVE_MAX_PER_RUN || 500);
  const maxDropPct = Number(process.env.RAKUTEN_MAX_CATALOG_DROP_PCT || 5);
  const maxMerchantDropPct = Number(process.env.CATALOG_MAX_MERCHANT_DROP_PCT || 25);
  const graceCutoff = new Date(Date.now() - graceDays * 24 * 60 * 60 * 1000).toISOString();
  const effectiveCutoff =
    cycleStartedAt && new Date(cycleStartedAt).getTime() < new Date(graceCutoff).getTime()
      ? cycleStartedAt
      : graceCutoff;

  // Snapshot + drop guard: refuse mass deactivation that would shrink the live catalog.
  const { count: beforeDisplayable, error: countErr } = await supabase
    .from(config.productsTable)
    .select('id', { count: 'exact', head: true })
    .eq('is_displayable', true);
  if (countErr) throw countErr;
  const baseline = Number(beforeDisplayable || 0);

  // Merchant baselines (sample of displayable) for per-MID drop gates.
  const merchantBaseline = {};
  {
    const { data: midRows } = await supabase
      .from(config.productsTable)
      .select('merchant_id')
      .eq('is_displayable', true)
      .not('merchant_id', 'is', null)
      .limit(20000);
    for (const r of midRows || []) {
      const mid = String(r.merchant_id || '').trim();
      if (!mid) continue;
      merchantBaseline[mid] = (merchantBaseline[mid] || 0) + 1;
    }
  }

  await supabase.from('system_status').upsert({
    key: 'catalog_last_known_good',
    value_json: {
      displayable: baseline,
      capturedAt: new Date().toISOString(),
      source: 'pre_mark_inactive',
      graceDays,
      effectiveCutoff,
      merchantBaseline,
    },
    updated_at: new Date().toISOString(),
  });

  let total = 0;
  const markedByMerchant = {};

  for (const source of feedSources) {
    if (total >= maxPerRun) break;
    let sourceMarked = 0;
    while (total < maxPerRun) {
      const remaining = maxPerRun - total;
      const { data: stale, error: selectError } = await withRetry(async () => {
        const result = await supabase
          .from(config.productsTable)
          .select('id, merchant_id')
          .eq('feed_source', source)
          .eq('is_active', true)
          .lt('last_seen_at', effectiveCutoff)
          .limit(Math.min(INACTIVE_CHUNK_SIZE, remaining));
        if (result.error) throw result.error;
        return result;
      });
      if (selectError) throw selectError;
      if (!stale?.length) break;

      // Projected drop if this batch (and prior batches) leave displayable.
      const projected = baseline - (total + stale.length);
      const dropPct = baseline > 0 ? (100 * (baseline - Math.max(projected, 0))) / baseline : 0;
      if (baseline > 50 && dropPct > maxDropPct) {
        console.error(
          `CRITICAL: catalog drop guard blocked markInactive (${dropPct.toFixed(1)}% > ${maxDropPct}%). ` +
            `baseline=${baseline} alreadyMarked=${total} batch=${stale.length}`
        );
        await supabase.from('system_status').upsert({
          key: 'catalog_publish_blocked',
          value_json: {
            blocked: true,
            reason: 'catalog_drop_exceeds_threshold',
            dropPct,
            maxDropPct,
            baseline,
            alreadyMarked: total,
            at: new Date().toISOString(),
          },
          updated_at: new Date().toISOString(),
        });
        break;
      }

      // Per-merchant drop gate: skip IDs that would wipe a major MID abnormally.
      const allowed = [];
      for (const row of stale) {
        const mid = String(row.merchant_id || '').trim() || '_unknown';
        const base = Number(merchantBaseline[mid] || 0);
        const already = Number(markedByMerchant[mid] || 0);
        if (base >= 50) {
          const projectedMid = base - (already + 1);
          const midDrop = (100 * (base - Math.max(projectedMid, 0))) / base;
          if (midDrop > maxMerchantDropPct) {
            continue;
          }
        }
        allowed.push(row);
        markedByMerchant[mid] = already + 1;
      }

      if (!allowed.length) {
        console.error(
          `CRITICAL: merchant drop guard blocked remaining markInactive batch (source=${source})`
        );
        await supabase.from('system_status').upsert({
          key: 'catalog_publish_blocked',
          value_json: {
            blocked: true,
            reason: 'merchant_drop_exceeds_threshold',
            maxMerchantDropPct,
            markedByMerchant,
            merchantBaseline,
            at: new Date().toISOString(),
          },
          updated_at: new Date().toISOString(),
        });
        break;
      }

      const ids = allowed.map((p) => p.id);
      await withRetry(async () => {
        const { error } = await supabase
          .from(config.productsTable)
          // Soft status only — never DELETE. Grace window requires multi-day feed absence.
          .update({ is_active: false })
          .in('id', ids);
        if (error) throw error;
      });
      sourceMarked += ids.length;
      total += ids.length;
      if (stale.length < Math.min(INACTIVE_CHUNK_SIZE, remaining)) break;
    }
    console.log(
      `Marked ${sourceMarked} products inactive for ${source} (grace ${graceDays}d, cutoff ${effectiveCutoff})`
    );
  }

  return total;
}

function first(row, keys) {
  for (const key of keys) {
    const direct = row[key];
    if (direct !== undefined && direct !== null && String(direct).trim() !== '') return String(direct).trim();
    const foundKey = Object.keys(row).find((candidate) => normalizeKey(candidate) === key);
    if (foundKey && row[foundKey] !== undefined && row[foundKey] !== null && String(row[foundKey]).trim() !== '') return String(row[foundKey]).trim();
  }
  return undefined;
}

const OFFER_PRICE_CURRENT_KEYS = [
  'sale_price', 'current_price', 'offer_price', 'special_price', 'final_price',
  'discounted_price', 'price_sale', 'saleprice', 'currentprice',
];
const OFFER_PRICE_RETAIL_KEYS = [
  'retail_price', 'original_price', 'was_price', 'list_price', 'msrp',
  'regular_price', 'standard_price', 'compare_at_price', 'suggested_retail_price',
  'price_retail', 'product_retail_price', 'retailprice', 'originalprice',
];
const OFFER_PRICE_AMBIGUOUS_KEYS = ['price', 'product_price'];
const OFFER_DISCOUNT_AMOUNT_KEYS = ['discount_amount', 'discount', 'savings', 'you_save', 'amount_off'];
const OFFER_DISCOUNT_PERCENT_KEYS = ['discount_percent', 'percent_off', 'sale_percent', 'pct_off'];

function resolveOfferPrices(row) {
  const salePrice = cleanText(first(row, OFFER_PRICE_CURRENT_KEYS));
  const retailPrice = cleanText(first(row, OFFER_PRICE_RETAIL_KEYS));
  const ambiguous = cleanText(first(row, OFFER_PRICE_AMBIGUOUS_KEYS));
  const discountAmount = numberFromPrice(first(row, OFFER_DISCOUNT_AMOUNT_KEYS));
  const discountPercent = numberFromPrice(first(row, OFFER_DISCOUNT_PERCENT_KEYS));

  let current = salePrice || ambiguous || retailPrice;
  let original = retailPrice;

  const saleNum = numberFromPrice(salePrice);
  const retailNum = numberFromPrice(retailPrice);
  const ambNum = numberFromPrice(ambiguous);

  if (saleNum && retailNum && saleNum < retailNum) {
    current = salePrice;
    original = retailPrice;
  } else if (saleNum && ambNum && saleNum < ambNum) {
    current = salePrice;
    original = ambiguous;
  } else if (ambNum && retailNum && ambNum < retailNum) {
    current = ambiguous;
    original = retailPrice;
  } else if (discountAmount > 0 && current) {
    const currNum = numberFromPrice(current);
    if (currNum) original = String(currNum + discountAmount);
  } else if (discountPercent >= 5 && current && !original) {
    const currNum = numberFromPrice(current);
    if (currNum) original = String(Math.round((currNum * 100) / (100 - discountPercent)));
  }

  if (!current) current = original;
  if (!original) original = current;

  return {
    price: current,
    originalPrice: original,
    rawPrice: first(row, [...OFFER_PRICE_CURRENT_KEYS, ...OFFER_PRICE_RETAIL_KEYS, ...OFFER_PRICE_AMBIGUOUS_KEYS]),
  };
}

async function refreshCatalogMaterializedViews(views = ['live_products_apparel_mat', 'live_products_footwear']) {
  const token = process.env.SUPABASE_ACCESS_TOKEN;
  const projectRef = process.env.SUPABASE_PROJECT_REF || 'burrylupizvggupsryuj';
  if (!token) {
    console.warn('[mv-refresh] SUPABASE_ACCESS_TOKEN missing — skipped');
    return [];
  }
  const refreshed = [];
  for (const view of views) {
    const concurrent = `REFRESH MATERIALIZED VIEW CONCURRENTLY public.${view};`;
    console.log(`[mv-refresh] ${concurrent}`);
    let res = await fetch(`https://api.supabase.com/v1/projects/${projectRef}/database/query`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ query: concurrent }),
    });
    if (!res.ok) {
      const blocking = `REFRESH MATERIALIZED VIEW public.${view};`;
      console.warn(`[mv-refresh] concurrent failed for ${view}, trying blocking refresh`);
      res = await fetch(`https://api.supabase.com/v1/projects/${projectRef}/database/query`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ query: blocking }),
      });
    }
    if (!res.ok) {
      const text = await res.text();
      throw new Error(`MV refresh failed for ${view}: ${text.slice(0, 300)}`);
    }
    refreshed.push(view);
    console.log(`[mv-refresh] OK ${view}`);
  }
  return refreshed;
}

/** Detect Canadian locale/shipping/currency signals in a Rakuten row (MID 43172 US+CA). */
function isCanadaOfferRow(row) {
  const locale = String(first(row, ['country', 'locale', 'ship_to_country', 'shipping_country']) || '').toUpperCase();
  const url = String(first(row, ['link', 'url', 'product_url', 'click_url']) || '').toLowerCase();
  const currencyRaw = String(first(row, ['currency', 'price_currency']) || '').toUpperCase();
  const priceStr = String(first(row, ['price', 'sale_price', 'current_price']) || '');
  if (locale === 'CA' || locale === 'CAN' || locale.includes('CANADA')) return true;
  if (/(\/ca\/|\.ca\/|country=ca|region=ca|locale=ca|\/en-ca\/)/.test(url)) return true;
  if (currencyRaw === 'CAD') return true;
  if (/C\$|\bCAD\b/i.test(priceStr)) return true;
  return false;
}

/** Resolve listing region + currency — MID 43172 splits US (USD) vs CA (CAD). */
function resolveMidRegionCurrency(mid, row, { mytheresa = false } = {}) {
  const midStr = String(mid || '').trim();
  const url = String(first(row, ['link', 'url', 'product_url', 'click_url', 'buy_url']) || '');
  const urlMidMatch = url.match(/[?&]mid=(\d+)/i);
  const urlMid = urlMidMatch ? urlMidMatch[1] : '';
  const effectiveMid = midStr || urlMid;
  let decodedUrl = url;
  try {
    decodedUrl = decodeURIComponent(url);
  } catch {
    // keep raw
  }

  // Affiliate deep links often carry the true MID even when the feed row mid is missing/wrong.
  if (
    effectiveMid === '43172' ||
    /[?&]mid=43172\b/i.test(url) ||
    /mytheresa\.com\/us\//i.test(decodedUrl)
  ) {
    if (isCanadaOfferRow(row)) {
      return { currency: 'CAD', region: 'ca', retailerCountry: 'CA' };
    }
    return { currency: 'USD', region: 'us', retailerCountry: 'US' };
  }

  if (effectiveMid === '43172') {
    if (isCanadaOfferRow(row)) {
      return { currency: 'CAD', region: 'ca', retailerCountry: 'CA' };
    }
    return { currency: 'USD', region: 'us', retailerCountry: 'US' };
  }

  const midOverride = MID_CURRENCY_MAP[effectiveMid] || MID_CURRENCY_MAP[midStr];
  if (midOverride) {
    const retailerCountry = midOverride.retailerCountry
      || first(row, ['country', 'locale'])
      || (mytheresa ? 'GB' : 'US');
    const currency = midOverride.currency
      || first(row, ['currency', 'price_currency'])
      || (retailerCountry === 'EU' ? 'EUR' : ['UK', 'GB'].includes(retailerCountry) ? 'GBP' : 'USD');
    return {
      currency,
      region: midOverride.region,
      retailerCountry,
    };
  }

  const retailerCountry = first(row, ['country', 'locale']) || (mytheresa ? 'GB' : 'US');
  const currency = first(row, ['currency', 'price_currency'])
    || (retailerCountry === 'EU' ? 'EUR' : ['UK', 'GB'].includes(retailerCountry) ? 'GBP' : 'USD');
  const region = currency === 'GBP' || ['UK', 'GB'].includes(retailerCountry) ? 'uk'
    : currency === 'EUR' || ['EU', 'DE', 'FR', 'IT', 'ES', 'NL'].includes(retailerCountry) ? 'eu'
      : isCanadaOfferRow(row) ? 'ca'
        : 'us';
  return { currency, region, retailerCountry };
}

function normalizeKey(value) {
  return String(value).trim().toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_|_$/g, '');
}

function cleanText(value) {
  if (!value) return undefined;
  return String(value)
    .replace(/<[^>]*>/g, ' ')
    .replace(/[_-]+/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
}

function normalizeBrandName(value) {
  if (!value) return undefined;
  const compact = value.toLowerCase().replace(/[^a-z0-9]/g, '');
  const aliases = {
    alc: 'A.L.C.',
    oseree: 'Oséree',
    oseree: 'Oséree',
    therow: 'The Row',
    loropiana: 'Loro Piana',
    isabelmarant: 'Isabel Marant',
    fleurdumal: 'Fleur du Mal',
    smaxmara: 'Weekend Max Mara',
    weekendmaxmara: 'Weekend Max Mara',
    maxmara: 'Max Mara',
    missoni: 'Missoni',
    missonimare: 'Missoni',
    ragandbone: 'rag & bone',
    ragbone: 'rag & bone',
    lagence: "L'Agence",
    redone: 'RE/DONE',
    khaite: 'Khaite',
    toteme: 'Toteme',
    vince: 'Vince',
    theory: 'Theory',
    equipment: 'Equipment',
    zimmermann: 'Zimmermann',
    agolde: 'AGOLDE',
    citizensofhumanity: 'Citizens of Humanity',
    ullahjohnson: 'Ulla Johnson',
    ullajohnson: 'Ulla Johnson',
    faithfullthebrand: 'Faithfull the Brand',
    anothertomorrow: 'Another Tomorrow',
    anine: 'Anine Bing',
    aninebing: 'Anine Bing',
    staud: 'Staud',
    officinegeneral: 'Officine Générale',
    officinegenerale: 'Officine Générale',
    jacquemus: 'Jacquemus',
    sandro: 'Sandro',
    maje: 'Maje',
    ba7: 'ba&sh',
    bash: 'ba&sh',
    marysia: 'Marysia',
    nili: 'Nili Lotan',
    nililotan: 'Nili Lotan',
    diesel: 'Diesel',
  };
  return aliases[compact] || titleCase(value);
}

function normalizeCategory(value, title = '') {
  const text = [value, title].filter(Boolean).join(' ').toLowerCase();
  if (text.includes('blazer')) return 'Blazer';
  if (text.includes('trouser') || text.includes('pants')) return 'Trousers';
  if (text.includes('skirt')) return 'Skirt';
  if (text.includes('dress')) return 'Dress';
  if (text.includes('shirt') || text.includes('blouse')) return 'Shirt';
  if (text.includes('coat')) return 'Coat';
  if (text.includes('jacket')) return 'Jacket';
  if (text.includes('knit') || text.includes('sweater') || text.includes('jumper')) return 'Knitwear';
  return value ? titleCase(value) : undefined;
}

function refineProductTitle(value, category, composition) {
  const cleaned = cleanText(value);
  const text = [cleaned, category, composition].filter(Boolean).join(' ').toLowerCase();
  if (text.includes('boatneck') || text.includes('boat neck')) return text.includes('cashmere') ? 'Cashmere Boatneck Knit' : 'Boatneck Knit';
  if (text.includes('slip') && text.includes('dress')) return text.includes('silk') ? 'Silk Slip Dress' : 'Slip Dress';
  if (text.includes('midi') && text.includes('dress')) return text.includes('silk') ? 'Silk Satin Midi Dress' : 'Midi Dress';
  if (text.includes('coat') && text.includes('wool')) return 'Structured Wool Coat';
  if (text.includes('blazer') && text.includes('wool')) return 'Structured Wool Blazer';
  if (text.includes('shirt') && text.includes('cotton')) return 'Cotton Poplin Shirt';
  if (text.includes('trouser') && text.includes('linen')) return 'Tailored Linen Trouser';
  return cleaned ? titleCase(cleaned) : undefined;
}

function titleCase(value) {
  return String(value)
    .toLowerCase()
    .split(' ')
    .filter(Boolean)
    .map((word) => ['and', 'of', 'in', 'with'].includes(word) ? word : word.charAt(0).toUpperCase() + word.slice(1))
    .join(' ');
}

function slugify(value) {
  if (!value) return undefined;
  return String(value).toLowerCase().replace(/&/g, 'and').replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
}

function isSale(price, originalPrice, row, { bloomingdales = false } = {}) {
  const saleFlag = first(row, ['is_sale', 'sale', 'on_sale']);
  if (saleFlag) return ['true', 'yes', '1', 'sale'].includes(String(saleFlag).toLowerCase());

  // Prefer explicit sale/retail pair when present (MyTheresa JSON feeds).
  const salePrice = numberFromPrice(first(row, ['sale_price', 'price', 'current_price']) || price);
  const retailPrice = numberFromPrice(
    first(row, ['retail_price', 'original_price', 'was_price', 'list_price', 'msrp']) || originalPrice
  );
  if (salePrice && retailPrice && salePrice < retailPrice) return true;

  const current = numberFromPrice(price);
  const original = numberFromPrice(originalPrice);
  if (current && original && current < original) return true;

  // Some merchants only send a discount percent / amount with equal price fields.
  const discount = first(row, OFFER_DISCOUNT_PERCENT_KEYS);
  const discountNum = Number(String(discount || '').replace(/[^0-9.]/g, ''));
  if (Number.isFinite(discountNum) && discountNum >= 5) return true;

  const discountAmt = numberFromPrice(first(row, OFFER_DISCOUNT_AMOUNT_KEYS));
  if (Number.isFinite(discountAmt) && discountAmt > 0) return true;

  // MyTheresa / retailer sale URL paths when price fields are identical.
  const url = String(first(row, ['url', 'link', 'buy_url', 'product_url']) || '').toLowerCase();
  if (url && /([?&]sale=|\/sale\/|%2fsale%2f|onsale|on-sale|markdown)/.test(url)) return true;

  const category = String(first(row, ['category', 'product_type', 'producttype']) || '').toLowerCase();
  const secondary = String(first(row, ['secondary_category', 'merchant_category']) || '').toLowerCase();
  const categoryBlob = `${category} ${secondary}`;
  if (categoryBlob && /\bsale\b|\boutlet\b|\bmarkdown\b|\bclearance\b|\breduced\b|\bpromotion\b/.test(categoryBlob)) {
    return true;
  }

  // Bloomingdale's XML often tags markdown via secondary category, not URL path.
  if (bloomingdales && categoryBlob && /\bfurther reduced\b|\bnow\s+\d+%\s+off\b/.test(categoryBlob)) {
    return true;
  }

  return false;
}

function numberFromPrice(value) {
  const match = String(value || '').match(/[0-9]+(?:\.[0-9]+)?/);
  return match ? Number(match[0]) : undefined;
}

const STANDARD_COLORS = new Set([
  'black', 'white', 'ivory', 'cream', 'ecru', 'off-white', 'neutrals', 'beige', 'grey', 'navy', 'blue',
  'red', 'burgundy', 'pink', 'green', 'brown', 'orange', 'yellow', 'gold', 'silver', 'rose gold',
  'metallic', 'purple', 'print', 'animal print', 'multi',
]);

const COLOR_VALUE_MAP = {
  black: 'black', noir: 'black', onyx: 'black', ebony: 'black', midnight: 'black',
  white: 'white', 'optical white': 'white', 'bright white': 'white', snow: 'white',
  ivory: 'ivory',
  cream: 'cream', oat: 'cream', butter: 'cream', vanilla: 'cream',
  ecru: 'ecru',
  'off white': 'off-white', 'off-white': 'off-white',
  nude: 'neutrals', neutral: 'neutrals', natural: 'neutrals', 'blush nude': 'neutrals',
  beige: 'beige', sand: 'beige', stone: 'beige', taupe: 'beige', camel: 'beige', khaki: 'beige',
  grey: 'grey', gray: 'grey', charcoal: 'grey', slate: 'grey', ash: 'grey', pewter: 'grey', smoke: 'grey',
  navy: 'navy', 'midnight blue': 'navy', 'dark navy': 'navy', ink: 'navy',
  blue: 'blue', cobalt: 'blue', cerulean: 'blue', cornflower: 'blue', teal: 'blue', denim: 'blue', indigo: 'blue', sky: 'blue', 'powder blue': 'blue',
  red: 'red', scarlet: 'red', cherry: 'red', crimson: 'red', tomato: 'red', rouge: 'red',
  burgundy: 'burgundy', bordeaux: 'burgundy', wine: 'burgundy', oxblood: 'burgundy', maroon: 'burgundy', merlot: 'burgundy',
  pink: 'pink', blush: 'pink', rose: 'pink', mauve: 'pink', fuchsia: 'pink', coral: 'pink', peach: 'pink', flamingo: 'pink', 'ballet pink': 'pink',
  green: 'green', olive: 'green', sage: 'green', forest: 'green', emerald: 'green', mint: 'green', moss: 'green', jade: 'green', hunter: 'green', 'khaki green': 'green',
  brown: 'brown', chocolate: 'brown', cognac: 'brown', tobacco: 'brown', toffee: 'brown', cinnamon: 'brown', chestnut: 'brown', walnut: 'brown', tan: 'brown',
  orange: 'orange', terracotta: 'orange', rust: 'orange', amber: 'orange', tangerine: 'orange', apricot: 'orange', 'burnt orange': 'orange',
  yellow: 'yellow', mustard: 'yellow', saffron: 'yellow', lemon: 'yellow', canary: 'yellow',
  gold: 'gold', golden: 'gold', champagne: 'gold', bronze: 'gold',
  silver: 'silver',
  'rose gold': 'rose gold',
  metallic: 'metallic', sequin: 'metallic', lurex: 'metallic', shimmer: 'metallic', glitter: 'metallic',
  purple: 'purple', violet: 'purple', lavender: 'purple', lilac: 'purple', plum: 'purple', aubergine: 'purple', grape: 'purple',
  print: 'print', floral: 'print', stripe: 'print', striped: 'print', check: 'print', plaid: 'print', paisley: 'print', printed: 'print', houndstooth: 'print', gingham: 'print',
  'animal print': 'animal print', leopard: 'animal print', cheetah: 'animal print', zebra: 'animal print', snakeskin: 'animal print', python: 'animal print',
  multi: 'multi', multicolor: 'multi', multicolour: 'multi', colorblock: 'multi', 'colour block': 'multi', rainbow: 'multi',
};

const COLOR_MAP_KEYS_LONGEST_FIRST = Object.keys(COLOR_VALUE_MAP).sort((a, b) => b.length - a.length);

function normalizeColorValue(raw) {
  if (!raw) return undefined;
  const v = String(raw).trim().toLowerCase()
    .replace(/[^a-z0-9\s\-]/g, '')
    .replace(/\s+/g, ' ')
    .trim();
  if (!v) return undefined;

  if (COLOR_VALUE_MAP[v]) return COLOR_VALUE_MAP[v];

  for (const key of COLOR_MAP_KEYS_LONGEST_FIRST) {
    if (v.includes(key)) return COLOR_VALUE_MAP[key];
  }

  return STANDARD_COLORS.has(v) ? v : undefined;
}

function parseSizeOptions(row) {
  const raw = first(row, ['size_options', 'sizes', 'size', 'available_sizes']);
  if (!raw) return undefined;
  const parts = String(raw)
    .split(/[|,/;]/)
    .map((s) => s.trim())
    .filter(Boolean);
  return parts.length ? unique(parts).slice(0, 30) : undefined;
}

function inferCompositionFromRow(row) {
  const description = cleanText(first(row, [
    'description', 'short_desc', 'long_description', 'product_description',
    'material', 'fabric', 'material_description', 'fabric_description',
  ]));
  if (!description) return undefined;

  // Prefer explicit percent + fiber (e.g. "crafted in 100% cotton denim…").
  const percentFiber = description.match(
    /(\d+\s*%\s*(?:organic\s+)?(?:cotton|linen|silk|wool|cashmere|alpaca|mohair|hemp|ramie|lyocell|tencel|viscose|cupro|leather|suede)(?:\s*[\/&]\s*(?:organic\s+)?(?:cotton|linen|silk|wool|cashmere|alpaca|mohair|hemp|ramie|lyocell|tencel|viscose|cupro|leather|suede|\d+\s*%))?)/i
  );
  if (percentFiber?.[1]) {
    const candidate = stripCompositionPrefix(percentFiber[1].trim());
    if (isValidCompositionString(candidate)) return candidate;
  }

  const labeled = description.match(
    /(?:material|fabric|composition|content|made of|crafted from|crafted in|made from|made in)\s*[:\-]?\s*([^.;\n]{3,160})/i
  );
  if (labeled?.[1]) {
    const candidate = stripCompositionPrefix(labeled[1].trim());
    // "cupro, a butter soft…" → normalize to "100% cupro" when no percent present
    const fiberOnly = candidate.match(
      /^(organic\s+)?(cotton|linen|silk|wool|cashmere|alpaca|mohair|hemp|ramie|lyocell|tencel|viscose|cupro|leather|suede)\b/i
    );
    if (fiberOnly && !candidate.includes('%')) {
      const normalized = `100% ${fiberOnly[2].toLowerCase()}`;
      if (isValidCompositionString(normalized)) return normalized;
    }
    if (isValidCompositionString(candidate)) return candidate;
  }

  const percentMatch = description.match(/(\d+\s*%\s*[a-z][^.;\n]{2,80})/i);
  if (percentMatch?.[1]) {
    const candidate = stripCompositionPrefix(percentMatch[1].trim());
    if (isValidCompositionString(candidate)) return candidate;
  }

  return undefined;
}

function resolveAvailability(row) {
  const availability = first(row, [
    'availability', 'in_stock', 'stock_status', 'shipping_availability',
    'quantity', 'stock_quantity', 'inventory', 'inventory_count',
  ]);
  const description = cleanText(first(row, ['description', 'short_desc', 'long_description'])) || '';
  const name = cleanText(first(row, ['name', 'title', 'product_name'])) || '';
  const combined = `${availability || ''} ${description} ${name}`.toLowerCase();

  if (/(out\s*of\s*stock|sold\s*out|unavailable|discontinued|not\s*available|no\s*longer\s*available)/.test(combined)) {
    return { isAvailable: false, stockStatus: 'sold_out' };
  }

  const stockStatus = inferStockStatus(availability);
  if (stockStatus === 'sold_out' || stockStatus === 'out_of_stock') {
    return { isAvailable: false, stockStatus };
  }
  if (stockStatus === 'in_stock' || stockStatus === 'low_stock') {
    return { isAvailable: true, stockStatus };
  }

  // Listed in merchant catalog feed with no explicit OOS signal → in stock.
  // Rakuten product files omit shipping.availability for most SKUs; presence in feed means sellable.
  return { isAvailable: true, stockStatus: 'in_stock' };
}

async function deactivateUnavailableProducts(supabase, table) {
  let total = 0;
  const patterns = ['sold_out', 'out_of_stock', 'unavailable', 'discontinued'];
  const MAX_DEACTIVATE_PER_RUN = Number(process.env.RAKUTEN_MAX_DEACTIVATE_PER_RUN || 2000);

  for (const status of patterns) {
    while (total < MAX_DEACTIVATE_PER_RUN) {
      const remaining = MAX_DEACTIVATE_PER_RUN - total;
      const { data, error } = await supabase
        .from(table)
        .update({ is_active: false })
        .eq('stock_status', status)
        .eq('is_active', true)
        .select('id')
        .limit(Math.min(INACTIVE_CHUNK_SIZE, remaining));
      if (error) throw error;
      const n = data?.length || 0;
      total += n;
      if (n < Math.min(INACTIVE_CHUNK_SIZE, remaining)) break;
    }
    if (total >= MAX_DEACTIVATE_PER_RUN) {
      console.warn(
        `deactivateUnavailableProducts capped at ${MAX_DEACTIVATE_PER_RUN} (RAKUTEN_MAX_DEACTIVATE_PER_RUN)`
      );
      break;
    }
  }

  return total;
}

/** No-op — stock_status is set on upsert from feed; do not bulk-reset to unknown. */
async function backfillStockStatus(_supabase, _table) {
  return 0;
}

function inferStockStatus(availability) {
  const text = String(availability || '').toLowerCase().trim();
  if (!text) return null;
  if (/(out\s*of\s*stock|sold\s*out|unavailable|not\s*available|discontinued|no\s*longer\s*available)/.test(text)) {
    return 'sold_out';
  }
  if (/(low\s*stock|limited|few\s*left)/.test(text)) return 'low_stock';
  if (/(in\s*stock|available|yes|true|1|pre-?order)/.test(text)) return 'in_stock';
  return null;
}

// Mirrors public.catalog_is_womens_apparel_category (20240020)
const WOMENS_APPAREL_TOKENS = [
  'dress', 'gown', 'shirt', 'blouse', 'top', 'tank', 'turtleneck', 'bodysuit',
  'knitwear', 'knit', 'sweater', 'jumper', 'cardigan', 'pullover', 'vest',
  'trouser', 'trousers', 'pants', 'pant', 'skirt', 'skort', 'shorts', 'jean', 'jeans', 'denim',
  'blazer', 'jacket', 'coat', 'outerwear', 'suit',
  'bottom', 'swimwear', 'lingerie', 'loungewear', 'sleepwear', 'robe', 'nightgown',
  'legging', 'jogger', 'overall', 'jumpsuit', 'romper',
  'parka', 'anorak', 'windbreaker', 'bomber', 'trench', 'poncho', 'cape',
  'kaftan', 'sarong', 'wrap', 'kimono', 'apparel', 'clothing',
  'women', 'woman', 'ladies', 'lady',
];

const CONSUMER_REJECTED_TOKENS = [
  'shoe', 'footwear', 'sandal', 'boot', 'sneaker', 'heel', 'pump', 'loafer', 'mule', 'flat',
  'bag', 'handbag', 'tote', 'clutch', 'purse', 'backpack', 'wallet',
  'jewelry', 'jewellery', 'earring', 'necklace', 'bracelet', 'brooch',
  'watch', 'watches',
  'accessory', 'accessories', 'belt', 'scarf', 'glove', 'sunglass', 'eyewear', 'glass',
  'cosmetic', 'beauty', 'fragrance', 'perfume', 'home', 'decor', 'furniture', 'candle',
  'lubricant', 'lube', 'supplement', 'vitamin', 'skincare', 'serum', 'moisturizer',
  'electronics', 'book', 'books', 'wellness', 'health', 'toy', 'toys', 'cleaner', 'care kit',
  'kid', 'kids', 'child', 'children', 'infant', 'baby',
  'pillow', 'pillows', 'cushion', 'bedding',
];

const NON_APPAREL_CATEGORY_TERMS = [
  'beauty', 'health', 'wellness', 'skincare', 'fragrance',
  'home', 'decor', 'electronics', 'books', 'toys', 'bedding',
];

const NON_APPAREL_NAME_TERMS = [
  'lubricant', 'lube', 'supplement', 'vitamin', 'serum',
  'moisturizer', 'perfume', 'fragrance', 'candle', 'diffuser',
  'pillow', 'pillows', 'throw pillow', 'decorative pillow', 'cushion',
];

function includesToken(haystack, token) {
  const h = String(haystack || '').toLowerCase();
  const n = String(token || '').toLowerCase().replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  if (!n) return false;
  // Match singular/plural word boundaries: trouser↔trousers, jean↔jeans
  return new RegExp(`(?:^|[^a-z0-9])${n}s?(?:[^a-z0-9]|$)`).test(h);
}

function isMensGenderLabel(gender = '') {
  const g = String(gender || '').toLowerCase();
  if (!g) return false;
  if (/\b(women|womens|woman|female|ladies|lady)\b/.test(g)) return false;
  return /\b(men|mens|male|man)\b/.test(g);
}

function isKidsGenderLabel(gender = '') {
  const g = String(gender || '').toLowerCase();
  return /\b(kid|kids|child|children|infant|baby|boy|girl)\b/.test(g);
}

function isWomensApparelCategory(category = '', name = '') {
  const cat = String(category || '').toLowerCase().trim();
  const nam = String(name || '').toLowerCase().trim();
  if (!cat && !nam) return false;

  for (const tok of CONSUMER_REJECTED_TOKENS) {
    if (includesToken(cat, tok) || includesToken(nam, tok)) return false;
  }

  if ((includesToken(cat, 'men') || includesToken(cat, 'mens') || includesToken(nam, ' for men') || includesToken(nam, ' mens '))
    && !includesToken(cat, 'women') && !includesToken(cat, 'woman')
    && !includesToken(nam, 'women') && !includesToken(nam, 'woman')) {
    return false;
  }

  return WOMENS_APPAREL_TOKENS.some((tok) => includesToken(cat, tok) || includesToken(nam, tok));
}

function consumerExclusionReason({ category = '', name = '', composition = '', imageUrl = '', price = '', url = '', gender = '' }) {
  if (!imageUrl || !String(imageUrl).trim()) return 'missing_image';
  const priceText = String(price || '').trim().toLowerCase();
  if (!priceText || ['n/a', 'na', '0', '0.00', '$0', '$0.00'].includes(priceText)) return 'missing_price';
  if (!url || !/^https?:\/\//i.test(String(url).trim())) return 'missing_url';
  if (!composition || !String(composition).trim()) return 'missing_composition';

  const cat = String(category || '').toLowerCase();
  const nam = String(name || '').toLowerCase();
  if (NON_APPAREL_NAME_TERMS.some((term) => nam.includes(term))) return 'non_apparel_product';
  if (NON_APPAREL_CATEGORY_TERMS.some((term) => cat.includes(term))) return 'non_apparel_product';

  if (/(shoe|footwear|sandal|boot|sneaker|heel|pump|loafer|mule)/.test(cat) || /(shoe|sandal|boot|sneaker|heel|pump|loafer|mule)/.test(nam)) return 'shoes';
  if (/(bag|handbag|tote|clutch|pouch|wallet|backpack)/.test(cat) || /(handbag|tote bag|clutch)/.test(nam)) return 'bags';
  if (/(jewelry|jewellery|earring|necklace|bracelet|brooch)/.test(cat) || /(earring|necklace|bracelet|brooch)/.test(nam)) return 'jewelry';
  if (cat.includes('watch') || nam.includes(' watch ') || nam.startsWith('watch ')) return 'watches';
  if (/(belt|scarf|hat|cap|glove|sunglass|eyewear|accessory|accessories)/.test(cat)) return 'accessories';
  if (isMensGenderLabel(gender)) return 'mens';
  if ((cat.includes('mens') || cat.startsWith('men') || nam.includes(' for men') || nam.includes(' mens '))
    && !cat.includes('women') && !cat.includes('woman')
    && !nam.includes('women') && !nam.includes('woman')) return 'mens';
  if (isKidsGenderLabel(gender) || /(kid|kids|child|children|girl|boy|baby|infant)/.test(cat)) return 'kids';
  if (/(beauty|fragrance|perfume|makeup|skincare|cosmetic|home|decor|furniture|candle|bedding)/.test(cat)
    || /(pillow|throw pillow|decorative pillow|cushion)/.test(nam)) return 'beauty_home';
  if (!isWomensApparelCategory(category, name)) return 'not_womens_apparel';
  return null;
}

function passesConsumerIngestionGate(fields) {
  return consumerExclusionReason(fields) === null;
}

/** Women's natural leather / suede footwear — stocks Natural Shoes rails. */
function passesFootwearIngestionGate({ category, name, composition, imageUrl, price, url, gender }) {
  if (!imageUrl || !price || !url) return false;
  const cat = String(category || '').toLowerCase();
  const nam = String(name || '').toLowerCase();
  const blob = `${cat} ${nam}`;
  if (!/(shoe|footwear|sandal|boot|sneaker|heel|pump|loafer|mule|trainer|espadrille|clog|ballerin)/.test(blob)) {
    return false;
  }
  if (/(kid|kids|child|children|infant|baby|junior|youth|\bboy\b|\bgirl\b)/.test(blob)) return false;
  if ((/\bmen\b|\bmenswear\b|\bmens\b/.test(cat) || /\bfor men\b/.test(blob))
    && !/(women|woman|female|ladies)/.test(blob)) {
    return false;
  }
  if (gender && /^men/i.test(gender) && !/(women|woman|female)/i.test(gender)) return false;
  const comp = String(composition || '').toLowerCase();
  if (comp) {
    return /(leather|suede|nubuck|lambskin|calfskin|shearling|cowhide|goatskin)/.test(comp);
  }
  return /(leather|suede|nubuck)/.test(nam);
}

function approvalStatusFootwear(nfp, composition = '', genderScope = 'unknown') {
  if (genderScope === 'men' || genderScope === 'kids') return 'pending';
  if (nfp != null && nfp >= 80) return 'yes';
  const text = String(composition || '').toLowerCase();
  if (/(leather|suede|nubuck|lambskin|calfskin|shearling)/.test(text)) return 'yes';
  return 'pending';
}

function passesRakutenXmlPrefilter(cl) {
  // Allow shoes through — footwear gate decides leather/suede eligibility later.
  if (/(handbag|tote bag|clutch|wallet|backpack|jewelry|jewellery|earring|necklace|bracelet|watch|perfume|fragrance|cosmetic|furniture|home decor|lubricant|supplement|vitamin|skincare|serum|moisturizer|electronics|book|wellness|health|toy|cleaner)/.test(cl)) {
    return false;
  }
  if ((/\bfor men\b|> men\b| men's | menswear|male apparel/.test(cl) || /<gender>\s*men/i.test(cl))
    && !/women|woman|female|ladies/i.test(cl)) {
    return false;
  }
  if (/\b(kids?|children|infant|baby|boys?|girls?)\b/.test(cl) && !/women|woman/i.test(cl)) return false;
  return /(clothing|apparel|dress|shirt|blouse|trouser|skirt|coat|blazer|jacket|sweater|knit|jumper|swimwear|lingerie|loungewear|women|woman|shoe|footwear|sandal|boot|sneaker|heel|pump|loafer)/.test(cl);
}

function stripCompositionPrefix(value) {
  if (!value) return value;
  // Strip leading 'Material: ' or 'Fabric: ' labels (Mytheresa / Rakuten XML format)
  let v = value.replace(/^(material|fabric|composition|content):\s*/i, '');
  // Truncate at care instructions or origin suffix: '. Care instructions', '. Made in', '. Item color'
  v = v.replace(/[.;,]\s*(care instructions|made in|item color|designer color|dry clean|machine wash|hand wash).*/i, '').trim();
  return v;
}

// Phrases that indicate a product description / fit description, not a fiber composition
const DESCRIPTION_PREFIXES = /^(fits like|feels like|originally released|made from|crafted from|this|the|a |an |our |style|designed|featuring|product|description|details|care|color:|colour:)/i;
const ALL_FIBER_WORDS = ['linen', 'silk', 'cashmere', 'wool', 'cotton', 'alpaca', 'mohair', 'hemp', 'ramie', 'jute', 'lyocell', 'tencel', 'viscose', 'cupro', 'leather', 'suede', 'shearling', 'polyester', 'acrylic', 'nylon', 'polyamide', 'spandex', 'elastane', 'lycra'];

function isValidCompositionString(value) {
  if (!value) return false;
  // Must be under 200 chars
  if (value.length >= 200) return false;
  // Reject if it starts with a product-description phrase
  if (DESCRIPTION_PREFIXES.test(value.trim())) return false;
  const lower = value.toLowerCase();
  // Must contain at least one fiber word
  if (!ALL_FIBER_WORDS.some((f) => lower.includes(f))) return false;
  // Must contain '%' OR be very short (<80 chars, e.g. "100% Cotton", "Silk", "Leather & Suede")
  return value.includes('%') || value.length < 80;
}

const NATURAL_FIBER_WORDS = ['linen', 'silk', 'cashmere', 'wool', 'cotton', 'alpaca', 'mohair', 'hemp', 'ramie', 'jute', 'lyocell', 'tencel', 'viscose', 'cupro', 'leather', 'suede', 'shearling'];
const SYNTHETIC_FIBER_WORDS = ['polyester', 'acrylic', 'nylon', 'polyamide', 'spandex', 'elastane', 'lycra', 'polypropylene', 'polylurethane', 'synthetic', 'man-made'];

function classifyGenderScope({ category = '', name = '', url = '', gender = '' }) {
  if (isKidsGenderLabel(gender)) return 'kids';
  const cat = String(category || '').toLowerCase();
  const nam = String(name || '').toLowerCase();
  const u = String(url || '').toLowerCase();
  if (isMensGenderLabel(gender)) return 'men';
  if ((cat.includes('mens') || cat.startsWith('men ') || nam.includes(' for men') || nam.includes(' mens '))
    && !cat.includes('women') && !nam.includes('women')) return 'men';
  if (u.includes('/men/') || u.includes('/mens/') || u.includes('/menswear/')) {
    if (!u.includes('women') && !u.includes('womens')) return 'men';
  }
  if (/\b(women|womens|woman|female|ladies)\b/i.test(`${gender} ${cat} ${nam}`)) return 'women';
  if (/\bunisex\b/i.test(gender)) return 'unisex';
  if (isWomensApparelCategory(category, name)) return 'women';
  return 'unknown';
}

function approvalStatus(nfp, composition = '', category = '', name = '', genderScope = 'unknown') {
  if (genderScope === 'men' || genderScope === 'kids') return 'pending';
  if (genderScope === 'unknown') return 'pending';
  if (!isWomensApparelCategory(category, name) && genderScope !== 'women' && genderScope !== 'unisex') {
    return 'pending';
  }
  // If we parsed a numeric fiber percent, use that as the definitive test
  if (nfp !== undefined && nfp !== null) {
    if (nfp >= 80) return 'yes';
    return 'pending';
  }
  // No percent data — only approve if composition lists ONLY natural fibers, no synthetics.
  // Cupro/viscose/lyocell alone are regenerated cellulose: require explicit NFP >= 80 instead.
  if (!composition) return 'pending';
  const text = composition.toLowerCase();
  const hasSynthetic = SYNTHETIC_FIBER_WORDS.some((f) => text.includes(f));
  if (hasSynthetic) return 'pending';
  const regeneratedOnly = /\b(cupro|viscose|lyocell|tencel|modal)\b/.test(text)
    && !/\b(cotton|linen|silk|wool|cashmere|alpaca|mohair|hemp|ramie|leather|suede)\b/.test(text);
  if (regeneratedOnly) return 'pending';
  const hasNatural = NATURAL_FIBER_WORDS.some((f) => text.includes(f));
  if (hasNatural) return 'yes';
  return 'pending';
}

const SECONDARY_COMPOSITION_RE = /(?:^|[.;]\s*|\n)\s*(rib(?:bed)?\s*parts?|rib(?:bed)?|trim|embroidery|lining|contrast|pocket(?:ing)?|button|decoration|appliqu[eé]|exclusive of|excluding)\s*:/i;

function splitCompositionSegments(composition = '') {
  let text = String(composition).replace(/<br\s*\/?>/gi, '\n');
  text = text.replace(/\.\s+(?=(rib(?:bed)?\s*parts?|rib(?:bed)?|trim|embroidery|lining|contrast|pocket(?:ing)?|button|decoration|appliqu[eé]|exclusive of|excluding)\s*:)/gi, '\n');
  return text.split(/\n|;|\|/).map((s) => s.trim()).filter(Boolean);
}

function isSecondaryCompositionSegment(segment = '') {
  return SECONDARY_COMPOSITION_RE.test(segment) || /^(trim|embroidery|lining|contrast|pocket|rib(?:bed)?(?:\s*parts?)?|button|decoration|appliqu[eé])\s*:/i.test(segment);
}

function naturalFiberPercent(composition = '') {
  const raw = String(composition || '');
  const text = raw.toLowerCase();
  // MyTheresa / marketplace shoe format without percentages: "Upper: Bovine leather, Sole: Rubber"
  if (!/\d+\s*%/.test(text)) {
    const upperMatch = text.match(/(?:upper|body|outer|main)\s*:\s*([^,;|]+)/);
    const upper = (upperMatch ? upperMatch[1] : text).trim();
    if (
      /(leather|suede|nubuck|lambskin|calfskin|shearling|goatskin|cowhide)/.test(upper) &&
      !/(polyurethane|polyester|synthetic|vegan|faux)/.test(upper)
    ) {
      return 100;
    }
    if (/(^|[^a-z])100\s*%\s*(leather|suede)/.test(text)) return 100;
  }

  const segments = splitCompositionSegments(composition);
  const bodySegment = segments.find((s) => !isSecondaryCompositionSegment(s)) || segments[0] || '';
  const matches = [...bodySegment.toLowerCase().matchAll(/(\d+(?:\.\d+)?)\s*%\s*([a-z ]+)/g)];
  if (matches.length === 0) return undefined;
  const sum = matches.reduce((total, match) => {
    const fiber = match[2].trim();
    return total + (NATURAL_FIBERS.some((natural) => fiber.includes(natural)) ? Number(match[1]) : 0);
  }, 0);
  return Math.min(100, Math.round(sum));
}

function buildTags(row, composition, category) {
  const text = [composition, category, first(row, ['keywords', 'tags', 'description'])].filter(Boolean).join(' ').toLowerCase();
  return unique(['dress', 'shirt', 'skirt', 'blazer', 'trousers', 'linen', 'silk', 'cotton', 'wool', 'cashmere', 'sale'].filter((term) => text.includes(term)));
}

const CANONICAL_TO_EDITORIAL = {
  'vacation-shop': 'vacation',
  'vacation-edit': 'vacation',
  'occasion-edit': 'evening',
  'silk-occasion': 'evening',
  'evening-edit': 'evening',
  'tailoring-edit': 'tailoring',
  'city-wardrobe': 'fall-edit',
  'the-white-edit': 'leather-edit',
  'the-leather-edit': 'leather-edit',
};

function addEditorialSlugs(slugs) {
  const result = new Set(slugs);
  for (const slug of slugs) {
    const editorial = CANONICAL_TO_EDITORIAL[slug];
    if (editorial) result.add(editorial);
  }
  return Array.from(result);
}

function buildCollectionSlugs(composition, category) {
  const text = [composition, category].filter(Boolean).join(' ').toLowerCase();
  const canonical = unique([
    text.includes('linen') ? 'linen-essentials' : null,
    text.includes('silk') ? 'the-silk-edit' : null,
    text.includes('cashmere') ? 'the-cashmere-edit' : null,
    text.includes('wool') || text.includes('blazer') ? 'tailoring-edit' : null,
    text.includes('dress') ? 'occasion-edit' : null,
    /resort|vacation|beach|swim|kaftan|caftan/i.test(text) ? 'vacation-shop' : null,
    /city|urban|commute/i.test(text) ? 'city-wardrobe' : null,
    /leather|suede|nappa|lambskin|shearling|calfskin/i.test(text) ? 'the-leather-edit' : null,
  ].filter(Boolean));
  return addEditorialSlugs(canonical);
}

function buildEditorialCategories(composition, category) {
  const text = [composition, category].filter(Boolean).join(' ').toLowerCase();
  return unique([
    text.includes('linen') ? 'Linen Essentials' : null,
    text.includes('silk') ? 'The Silk Edit' : null,
    text.includes('cashmere') ? 'The Cashmere Edit' : null,
    text.includes('wool') ? 'Wool Tailoring' : null
  ].filter(Boolean));
}

function stableId(value) {
  let hash = 0;
  const input = String(value || 'rakuten-product');
  for (let i = 0; i < input.length; i += 1) hash = ((hash << 5) - hash + input.charCodeAt(i)) | 0;
  return `rakuten-${Math.abs(hash)}`;
}

function parseList(value) {
  if (!value) return [];
  if (Array.isArray(value)) return value;
  return String(value).split(',').map((item) => item.trim()).filter(Boolean);
}

function chunk(items, size) {
  const chunks = [];
  for (let index = 0; index < items.length; index += size) chunks.push(items.slice(index, index + size));
  return chunks;
}

function unique(values) {
  return [...new Set(values.filter(Boolean))];
}

export {
  syncRakutenFeeds,
  refreshCatalogMaterializedViews,
  resolveOfferPrices,
  isWomensApparelCategory,
  consumerExclusionReason,
  passesConsumerIngestionGate,
  passesFootwearIngestionGate,
  naturalFiberPercent,
  approvalStatus,
  classifyGenderScope,
  flattenRakutenXMLProduct,
  normalizeRakutenProduct,
};
