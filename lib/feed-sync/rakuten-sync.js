...
async function syncRakutenFeeds(options = {}) {
  ...
  const nullColorByMerchant = new Map();

  async function merchantNullColorIds(sourceUrl) {
    const mid =
      sourceUrl.match(/\/(\d{4,6})\//)?.[1]
      || sourceUrl.match(/\/(\d{4,6})_/)?.[1]
      || null;
    if (!mid) return nullColorByMerchant.get('__all__') || new Set();

    if (nullColorByMerchant.has(mid)) return nullColorByMerchant.get(mid);

    const ids = await loadNullColorProductIds(supabase, config.productsTable, mid);
    nullColorByMerchant.set(mid, ids);
    return ids;
  }
  ...
  async function processRows(rows, sourceUrl) {
    ...
    const products = [];
    const colorUpdates = [];
    const nullColorProductIds = await merchantNullColorIds(sourceUrl);
    ...
    for (const row of rows) {
      ...
      if (product) {
        ...
        skippedOutOfScope += 1;
        if (consumerExclusionReason(extractRowGateFields(row, sourceUrl)) !== 'missing_composition') {
          continue;
        }
        const colorUpdate = extractColorUpdateCandidate(row, sourceUrl);
        if (colorUpdate && nullColorProductIds.has(colorUpdate.product_id)) {
...
function readConfig(options) {
  ...
  return {
    ...
    markInactive: options.markInactive === true,
...
async function loadNullColorProductIds(supabase, table, retailerMid = null) {
  const ids = new Set();
  const PAGE = 1000; // Supabase REST max per request
  let offset = 0;
  while (true) {
    const { data, error } = await withRetry(async () => {
      ...
      if (retailerMid) query = query.eq('retailer_mid', retailerMid);
  ...
  console.log(
    `Color update index${retailerMid ? ` (${retailerMid})` : ''}: ${ids.size} null-color products with composition`
  );
  return ids;
}
