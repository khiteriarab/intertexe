...
function resolveAvailability(row) {
  const availability = first(row, ['availability', 'in_stock', 'stock_status', 'shipping_availability']);
  const description = cleanText(first(row, ['description', 'short_desc', 'long_description'])) || '';
  const combined = `${availability || ''} ${description}`.toLowerCase();
...
async function deactivateUnavailableProducts(supabase, table) {
  ...
  total += missingComposition?.length || 0;
