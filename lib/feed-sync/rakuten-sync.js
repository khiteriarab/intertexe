...
const MID_CURRENCY_MAP = {
  '35663': { currency: 'GBP', region: 'uk', retailerCountry: 'GB' },
...
function normalizeRakutenProduct(row, feedUrl) {
  ...
  const brandName = normalizeBrandName(cleanText(rawBrand));
...
function passesRakutenXmlPrefilter(cl) {
  if (/(shoe|footwear|sandal|boot|sneaker|handbag|tote bag|clutch|wallet|backpack|jewelry|jewellery|earring|necklace|bracelet|watch|perfume|fragrance|cosmetic|furniture|home decor)/.test(cl)) {
    return false;
  }
