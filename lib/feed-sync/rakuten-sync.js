  ...
  async function processRows(rows, sourceUrl) {
    ...
    for (const row of rows) {
      ...
      if (product) {
        ...
        if (colorUpdate) colorUpdates.push(colorUpdate);
      }
    }
    ...
    if (colorUpdates.length) {
      ...
      for (let i = 0; i < colorUpdates.length; i += CHUNK_SIZE) {
        const colorChunk = colorUpdates.slice(i, i + CHUNK_SIZE);
        for (const update of colorChunk) {
          const applied = await withRetry(async () => {
            const { data, error } = await supabase
              .from(config.productsTable)
              .update({ color: update.color, color_normalized: update.color })
