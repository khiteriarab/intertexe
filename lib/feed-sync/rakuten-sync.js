...
async function syncRakutenFeeds(options = {}) {
  ...
  const nullColorProductIds = await loadNullColorProductIds(supabase, config.productsTable);
...
async function loadNullColorProductIds(supabase, table) {
  const ids = new Set();
  const PAGE = 1000;
  let offset = 0;
  while (true) {
  ...
  console.log(`Color update index: ${ids.size} products with null color and known composition`);
  return ids;
}
