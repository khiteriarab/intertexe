...
import { CUSTOMER_ZERO_SLUG, DEMO_ORGANIZATION_SLUG, technicalPrincipalEmail } from "./constants";
// DEMO_ORGANIZATION_SLUG: demo SaaS tenant (not a consumer designer)
...
export type ProvisionOrganizationOperatorResult =
  | {
      status: "provisioned";
      profileId: string;
      authUserId: string;
...
export async function provisionOrganizationOperator(input: {
  client: SupabaseClient;
...
}): Promise<ProvisionOrganizationOperatorResult> {
  const email = input.email.trim().toLowerCase();
  const fullName = input.fullName.trim();
  const role = input.role.trim();
  if (!email || !fullName) throw new Error("Email and full name are required.");
  if (!ALLOWED_ROLES.has(role)) throw new Error("Role is not allowed for organization operators.");
  ...
  if (org.slug === CUSTOMER_ZERO_SLUG || org.slug === DEMO_ORGANIZATION_SLUG) {
    throw new Error("Provisioning is not allowed for reserved organizations.");
  }
  if (isTechnicalPrincipalEmail(email)) {
    throw new Error("Technical principal emails cannot be used for organization operators.");
  }
  ...
  if (authUserId) {
    if (await getIdentityLinkByEnterpriseUserId(authUserId)) {
      throw new Error("This Enterprise account is linked to HQ staff and cannot be used as an organization operator.");
    }
  } else {
  ...
  const otherOrgs = (memberships || [])
    ...
    .filter((row) => row.slug !== CUSTOMER_ZERO_SLUG && row.slug !== DEMO_ORGANIZATION_SLUG);
