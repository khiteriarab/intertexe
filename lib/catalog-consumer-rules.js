...
function isSyntheticLeatherProduct({ name = '', composition = '' } = {}) {
  const text = `${name} ${composition}`.trim();
  return text ? SYNTHETIC_LEATHER.test(text) : false;
}
...
export function consumerExclusionReason({
  ...
  if (isSyntheticLeatherProduct({ name, composition })) return 'synthetic_leather';

  // Natural leather / suede footwear is in-catalog (Manolo, Roger Vivier, etc.).
  // Only exclude shoes that fail the natural-upper bar.
  const isFootwear =
    /(shoe|footwear|sandal|boot|sneaker|heel|pump|loafer|mule|slipper|espadrille)/.test(cat) ||
    /(shoe|sandal|boot|sneaker|heel|pump|loafer|mule|slipper)/.test(nam);
  if (isFootwear) {
    const comp = String(composition || '').toLowerCase();
    const hasNaturalUpper =
      /(leather|suede|calfskin|goat|lamb|bovine|nappa|croc|python|raffia|silk|linen|cotton|wool|cashmere)/.test(
        comp
      ) ||
      /(leather|suede)/.test(nam);
    if (!hasNaturalUpper) return 'shoes';
    // Footwear is allowed — skip apparel-only checks below.
    if (/(kid|kids|child|children|toddler|infant|baby)/.test(cat) || /(kid|kids|toddler|infant)/.test(nam)) {
      return 'kids';
    }
    if (
      (cat.includes('mens') || cat.startsWith('men') || nam.includes(' for men') || nam.includes(' mens ')) &&
      !cat.includes('women') &&
      !cat.includes('woman') &&
      !nam.includes('women') &&
      !nam.includes('woman')
  ...
  if (/(sheet|pillowcase|duvet|bedding|towel|blanket|comforter|quilt|coverlet|bedspread|sham)/.test(nam)) return 'home';
