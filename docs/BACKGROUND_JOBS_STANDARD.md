...
5. **Deployment checklist (automatic).** `npm run check:background-jobs` runs on every Vercel build (`npm run build`) and via unit tests. It verifies:
   ...
   GitHub Actions workflow template: [`docs/BACKGROUND_JOBS_GATE.workflow.yml`](./BACKGROUND_JOBS_GATE.workflow.yml) (copy to `.github/workflows/` when a token with `workflow` scope is available).
