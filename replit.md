...
- **Backend:** All API routes fully migrated to Next.js API routes in `app/api/`. Express.js server in `server/` retained for shared modules (db.ts, storage.ts, auth helpers). Next.js API routes handle: auth (signup/login/logout/me), designers, products, shop, sitemap, recommend, favorites, product-favorites, quiz, scan-url, scan-tag, conversations (chat).
