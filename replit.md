...
- **AI Integration:** Utilizes OpenAI's GPT-4o-mini for AI-driven material advice, quiz recommendations, and scanner vision analysis, supporting conversation persistence.
...
- **Product Data:** Integrates a robust system for product verification, data scraping (Shopify JSON endpoints), and categorization. All products must be 80%+ natural fiber — viscose/rayon/modal/lyocell/acetate are classified as SYNTHETIC and do NOT count toward the natural fiber percentage. Every product query in `lib/supabase-server.ts` enforces `.gte("natural_fiber_percent", 80)`. It includes a `price_watches` system for price drop alerts.
- **Data Quality Cron:** A daily Vercel cron job (`/api/cron/data-quality`, runs at 3 AM UTC) automatically recalculates natural fiber percentages, corrects category misclassifications (skirts→bottoms, jackets→outerwear), removes duplicates, and flags products below the 80% threshold or missing images. Returns a JSON report of all fixes. Note: Hobby plan only supports daily crons — hourly schedule (`0 * * * *`) will cause deployment warnings.
- **SEO Optimization:** Dynamic rendering with `revalidate = 0` (no ISR caching) on all pages — content is always fresh. Sitemap index at `/api/sitemap` splits into pages, designers (1,000/page, 11,924 total), and product chunks (1,000/page, 15,976 total). `robots.txt` points to `https://www.intertexe.com/api/sitemap`. Designers with fabric data get higher sitemap priority (0.7) vs without (0.4). All sitemap entries include `lastmod` dates. Organization + WebSite JSON-LD on every page. Brand name "INTERTEXE" in H1, title tags, meta descriptions, and footer content. Canonical tags on all pages. Natural fiber percentages always shown as actual values (no rounding).
...
- **In-Store Scanner Pipeline:** Unified modular scanner at `app/api/scan/route.ts` with three input modes: photo (tag/garment via GPT-4o-mini Vision), barcode (BarcodeDetector API on mobile), and URL (Shopify JSON + HTML scraping + AI extraction). Pipeline stages: Input → Data Extraction → Fiber Analysis → Product Identification → Alternative Search → Output. Features include: multi-language fiber detection (Spanish, French, Italian, German), fiber classification (natural/semi-synthetic/synthetic), quality scoring, confidence system with user confirmation flow for low-confidence results, brand catalog average fallback, and diversified alternative recommendations across brands. Frontend at `app/scanner/ScannerClient.tsx` with tabbed interface. Legacy endpoints: `app/api/scan-tag/route.ts` (photo only), `app/api/scan-url/route.ts` (URL only).
